# EEG–Text 跨被试对齐小论文统一规格（v4.1-R4ALIGN / R6-EQALIGN-MSP：仓库对齐版）

**唯一研究主线 EQ-ANMA：受约束、可回退、fit-only 的对齐训练控制器**

> 版本：**v4.1-R4ALIGN（R6_EQALIGN_MSP）**，2026-08-24
...
> 对齐基线：`research/real-sham-r4-orthogonal-inner@954cecd5d8885bb274dd4cde97db6255bd9cf54d`
> 状态：**protocol economization + repository reconciliation，非 estimand 变更**。primary estimand 的科学内容与 v4.0 一致；本版额外修正了 v4.1 对仓库祖先、R1–R5 lineage 和当前可执行任务的错误假设。
> 本版不读取、不生成任何 EEG 数值、训练输出、held-out model metric、Gate 结果或 paper-level outcome。

> **权威性声明（R4ALIGN）**：本文是在上传的 v4.1 原文上做的仓库对齐修订；上传原文保持历史不可变。本修订以对齐基线的 `PROJECT_STATE.yaml`、`HANDOFF.md`、`TASKS.yaml` 和正式诊断 artifact 为事实来源。若当前 checkout 的 HEAD、分支或这些状态文件与本节不一致，先停止并报 `STATE_SPEC_CONFLICT`，不得猜测、合并或将 checkout 强行切到 `main`。
...
*文档结束。v4.1-R4ALIGN 未读取、未生成任何 EEG 数值、训练输出、held-out model metric、Gate 结果或 paper-level outcome；未修改任何已冻结的历史 artifact；未改写任何历史结论。相对 v4.0 的全部偏离已逐条记录于 §30，新增的仓库对齐偏离记录于 §0.3–0.5 与附录 D。*
