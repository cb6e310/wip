# EEG–Text 跨被试对齐小论文统一规格（v4.1-R6FREEZE-MAIN / R6-EQALIGN-MSP）

**唯一研究主线 EQ-ANMA：受约束、可回退、fit-only 的对齐训练控制器**

> 版本：**v4.1-R6FREEZE-MAIN（R6_EQALIGN_MSP）**，2026-08-24
...
> 对齐基线：`main@0a140bafabf9ec489547dda002f7613cafdfa4db`（R4 历史线已完成 fast-forward）
> 状态：**protocol economization + repository reconciliation + author-approved design freeze preparation，非 estimand 变更**。primary estimand 的科学内容与 v4.0 一致；本版把已批准的 R6 协议冻结绑定到当前 `main`，不释放任何实验执行。
> 本版不读取、不生成任何 EEG 数值、训练输出、held-out model metric、Gate 结果或 paper-level outcome。

> **权威性声明（R6FREEZE-MAIN）**：本文继承 R4ALIGN 与 MAIN-APPROVED 规格；R4→`main` 的 fast-forward 已在 `0a140bafabf9ec489547dda002f7613cafdfa4db` 完成。上传的 v4.1 原文、R4ALIGN 文件和 MAIN-APPROVED 文件保持历史不可变。本修订以 `PROJECT_STATE.yaml`、`HANDOFF.md`、`TASKS.yaml`、正式诊断 artifact 和当前 `main` tree 为事实来源。此文件只冻结 R6 协议与执行边界，不读取 EEG、不训练模型、不生成 held-out 指标；Codex 必须在 `main` 上提交本冻结后，才可开始实现任务。

> **当前状态覆盖（优先于本文的历史 R4ALIGN 段落）**：`main@0a140baf...` 是当前唯一未来工作基线；`R4_STATE_RECONCILIATION` 与 main activation 均已完成。本文中的 R4 branch-local 执行步骤（尤其 §0.5、D 节）仅作为不可变历史记录，不是当前行动指令。当前任务是 `R6_AUTHOR_FREEZE_ON_MAIN`（protocol-only），完成后唯一下一项是 `R6_IMPLEMENT_ARMS_AND_TESTS`；真实 R6 runner 尚未存在，禁止以 synthetic benchmark 代替真实实验。
...
### 0.5 当前可执行任务（R6FREEZE-MAIN）

`R4_STATE_RECONCILIATION` 与 R4→`main` 激活均已完成，作者已批准创建 R6
author freeze。当前唯一任务是：

`R6_AUTHOR_FREEZE_ON_MAIN` — 在
`main@0a140bafabf9ec489547dda002f7613cafdfa4db` 上提交本规格、冻结 artifact、
状态文件与 run record。它只冻结协议，不读取 EEG、不训练、不评分，也不修改
R0–R4 formal artifact 或 `TASKS.yaml`。

提交验证完成后，唯一下一项为 `R6_IMPLEMENT_ARMS_AND_TESTS`，继续在 `main`
工作；不得新建研究分支。真实 R6 runner 当前不存在，禁止把已有 synthetic
benchmark 当作 R6 实验。
...
**R6FREEZE-MAIN 覆盖条款（优先于本节旧的 A/B 默认叙述）**：R4 状态对齐、
作者审阅和 main activation 均已完成。当前 Stage 0/Stage A 的协议冻结绑定到
`main@0a140baf...`，并在 `eqalign_r6_author_freeze.yaml` 中引用 R4 历史分支与
formal SHA provenance；不得把 R4 inner diagnostic 当作 R6 held-out outcome。
本冻结不生成新的 R6 input hash、不跑单位成本训练、不读 outer。提交后才可进入
`R6_IMPLEMENT_ARMS_AND_TESTS`，仍在 `main` 上工作。
...
execution_order:
  ...
  - R6_AUTHOR_FREEZE_ON_MAIN         # current; protocol-only, no experiment
  - R6_IMPLEMENT_ARMS_AND_TESTS      # next only after freeze commit
  - R6_INNER_SELECTION
...
current_repository_gate:
  branch: main
  head: 0a140bafabf9ec489547dda002f7613cafdfa4db
  validator_provenance_branch: research/real-sham-r4-orthogonal-inner
  current_documentation_action: R6_AUTHOR_FREEZE_ON_MAIN
  ...
  r6_release: PREPARED_FOR_CODEX_COMMIT
  r6_experiment_blocked_until_freeze_commit: true
  next_after_freeze_commit: R6_IMPLEMENT_ARMS_AND_TESTS
```
...
## Main branch activation and author approval
...
- 作者批准的范围仅为：在 `main` 上创建新的 `R6_AUTHOR_FREEZE`；
- 本批准不释放 R4 outer confirmation、calibration、EQ-ANMA、Gate、A3 或 ROAMM；
- R6 实验运行必须等待新的 author freeze、数据消费范围、base commit 和执行合同
  在 `main` 上提交；
- 下一项任务：`R6_AUTHOR_FREEZE_ON_MAIN`；
- 若合并前后状态、formal SHA 或 read counters 不一致，必须停止并报告
  `STATE_SPEC_CONFLICT`。

## R6 author freeze on main（protocol-only；无实验）

本节是当前 `main` 状态下的 author freeze 绑定，优先于本文中描述 R4
branch-local 工作流的历史段落。它只固定未来 R6 的 estimand、臂、边界和预算，
...
4. 本冻结提交完成后，Codex 的唯一下一项实现任务是 `R6_IMPLEMENT_ARMS_AND_TESTS`，
   且必须继续在 `main` 上进行。

### F.4 冻结提交的验收

Codex 必须把本文件、`artifacts/eqalign_r6_author_freeze.yaml`、本次 run record、
`PROJECT_STATE.yaml`、`AI_START_HERE.md` 与 `HANDOFF.md` 一起提交到 `main`，并绑定
SPEC/artifact SHA。必须通过 `scripts/check_project_state.py`、
`scripts/project_status.py`、`git diff --check`；不得把 pytest 缺失误报为代码失败。

*文档结束。v4.1-R6FREEZE-MAIN 未读取、未生成任何 EEG 数值、训练输出、held-out model metric、Gate 结果或 paper-level outcome；未修改任何已冻结的历史 artifact；未改写任何历史结论。它只把作者批准的 R6 协议冻结绑定到 `main@0a140baf...`。*
