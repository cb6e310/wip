...
| **句子候选规模与可行性**【v3.11·D25–D28】 | v3.10 的 outer/inner source scope、source-slot identity、含等号 0.75–1.25 token 长度、MiniLM `>0.9`、\(H_{full}\) source identity、seed 20260813、\(L=5\) 与无回填全部保留。`711340d` 已准入 N=50 structural No-Go。ZuCo2 主人口改为每 scope `legal_count>=9` 的 common-support targets，主 \(N=10\)；只从已冻结 maximal ordering 取 9-negative prefix，不重算 tokenizer/embedding/cosine。显式落盘所有 excluded targets 与 coverage，不删训练 records | 任何 EEG 评测前生成 v3.11 N=10 common-support candidate view、paired 1:1/1:9 与 coverage audit；三个 v3.10 base artifacts 不可修改 |
...
16. 【v3.11·D25–D28】**逐 target 候选可行性已完成并触发 pre-outcome 协议修订**。`711340d` 证明 N=50 对 ZuCo2 structural unavailable（18,184/18,475 instances 不足 49）；这一 No-Go 永久保留在 deviation log。N=10 common-support 可保留 17,061/18,475=92.35% instances，因此本版将 `legal_count>=9` 冻结为显式评分人口。下一 artifact 只能投影已准入 base ordering；不得引入 train text、放宽过滤、回填或从训练 records 删除这些句子。如果投影后的精确 counts/hash 不符附录 O，必须作为 `STATE_SPEC_CONFLICT` 停止。
...
5. 外层 fold 清单；
6. 主指标、主 null、主候选集规模的预注册文件；outer、task-global 3×3 inner 与 v3.10 base candidate artifacts 已落盘准入，但在任何内层选参/Stage-1 前还必须生成并准入 v3.11 N=10 common-support scoring view；
...
13. 【v3.11】outer/inner split 与 v3.10 base candidate lists 已准入；不得把这些协议 artifacts 误写为“nested OOF 已就绪”。当前只允许按 D25–D28 派生 N=10 common-support view，不得由 Codex 自行更改 source/filter/eligibility/N 规则。
...
spec_version: v3_11_n10_common_support_2026_08_14  # D1--D28; no EEG/paper-level result inspected
...
> **v3.11 更新提示**：E.1–E.3 保留 v3.7 当时的 readiness 历史，不再是当前任务指令。text encoder、inner split 与 v3.10 base candidates 已准入；当前只执行附录 O.3 的 `S0_CANDIDATE_COMMON_SUPPORT`。其他数学定义和 No-Go 原则仍有效。
...
> **v3.11 历史提示**：M.1–M.4 记录 v3.9 的排序及已完成 inner-split 指令。D19/D20 的 ZuCo-first/ROAMM-deferred 裁决仍有效，但不得重复执行 M.4；附录 N.3 也已完成，当前只执行附录 O.3。
...
> **v3.11 历史提示**：N.1–N.3 已由提交 `711340d` 完成。其 N=50 审计规则与 `STRUCTURAL_NO_GO_N50` 结论作为 deviation evidence 永久保留，但 N.3 不得重跑；当前唯一指令见附录 O.3。
...
### O.3 唯一推荐的下一项 Codex 任务：`S0_CANDIDATE_COMMON_SUPPORT`

把下面整段连同 v3.11 ZIP 交给 Codex。附录 K/L/M/N 的旧 next-task 指令均已完成或失效，不得执行。
