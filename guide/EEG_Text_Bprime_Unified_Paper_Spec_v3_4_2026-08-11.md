...
| 项 | 规格 | 标记 |
|---|---|---|
| 特征定义 | 每个时间单元一个向量：通道 × 频带的带功率（θ1/θ2/α1/α2/β1/β2/γ1/γ2 八带，ZuCo 官方口径），维度 = 通道数 × 8（**暂按 \(105\times 8=840\) 写，须核实，§13.1 第 13 条**） | 【新·核】 |
...
| 项 | 规格 | 标记 |
|---|---|---|
| 权重 | LaBraM-Base 官方发布 checkpoint（**暂按「仅 Base 版本公开、参数量约 5.8M」写，须核实许可证与可得性，§13.1 第 15 条**） | 【核】 |
| **前置条件（一票否决）** | 其预训练语料**不得**包含 ZuCo 或任何自然阅读 EEG 语料。核实不通过 → A3 出局，触发 **CO-N7**（§11） | 【核】 |
| 预处理 | 带通 + 工频陷波 + 重采样至 **200 Hz**；通道按 EGI 128 → 该模型电极名列表映射，**必须显式提供输入通道顺序**，映射覆盖率进 T5 | 【核】 |
| 提取协议 | 沿用 arXiv:2606.06647 的口径：5 s × C 通道输入，取该模型 release 默认的 pooled embedding（LaBraM 为剥离 CLS token 后对 patch token 取均值），维度 **暂按 200 写（须核实）**；**不重新池化** | 【新·核】 |
...
- **item 定义**：以 content word type 或语义簇为 item（不以 subword token 为 item，否则响应矩阵过稀）。
...
- **红线**：通过支持门槛的 item 占比 \(<20\%\) → IRT 的 item discrimination 不可稳定估计 → 触发 Gate B 的 No-Go 预警，应把 item 粒度上移到语义簇。
...
4. 按 §6.15 实现并冻结 **ANMA-orig**（本文参考实现），交付其单元测试与观测统计（\(\bar y\)、\(\rho_{\rm band}\)、\(N_{\rm item}\) 选取结果）；**X1 已关闭，此项不再是 blocker**。SCI/CSPE 侧的可执行投影定义仍待核实（§13.1 第 3 条）。
...
| **A1 的频带/通道集合与切分版本**【v3.4】 | 八带 × 通道（暂按 840 维，待核实）；词级切分（主）+ 1 s 窗/0.5 s 步长固定窗（强制敏感性），两版并报 | Stage 0 |
...
| semantic item 粒度 | 必须有跨被试/item support | 数据卡完成后 |
...
13. 【补·v3.4】词级特征矩阵的**实际通道数与频带数**（本文档暂按 \(105\times 8=840\) 写，**未核实**）、字段命名与单位。
14. 【补·v3.4】**LaBraM / CBraMod / REVE 的预训练语料清单是否包含 ZuCo 或任何自然阅读 EEG**——若包含，联合留出被破坏，该模型直接出局（**CO-N7**）。**此条优先级高于其性能表现**，须在任何 T6 实验之前完成。
15. 【补·v3.4】LaBraM-Base checkpoint 的**许可证**、是否只有 Base 版本公开、参数量、pooled embedding 维度（本文档暂按 5.8M / 200 维写，**未核实**）；以及其要求的电极名列表、EGI 128 → 该列表的公开映射是否存在与覆盖率。
...
17. 【补·v3.4】arXiv:2606.06647 是否公开其 foundation model 特征提取代码——若有，A3 的适配可直接复用，省去一次口径自定义。
...
11. 【补】Gate A 统计被试群体的口径定义（附录 E-5）。
...
spec_version: v3_4_merged_2026_08_11        # v3.3 + J27 (backbone A selection: A1 primary / A3 secondary / A2 dropped) + J28 (T1 row-1 definition) + J29 (dual computation basis); closes blocker X2
...
**结论（v3.4 修订）：规格层已完备，先导已可开跑，前置只剩一项。** 准确表述是——**写作可开工四成，实现可开工八成，Gate A 在 item 粒度裁定完成后即可开工**。

v3.1 关闭了 X1（ANMA 原始定义，§6.15），**v3.4 关闭了 X2（backbone \(A\)，§4.7）**。因此 Gate A 的前置依赖由「两个 blocker + 六项形式化缺口」收缩为：**semantic item 粒度裁定**一项，加上 E.2 中仍未决的 **E-5** 一项（E-1 至 E-4 已由附录 F 裁定，E-6 已由 v3.4 回填 §4.6）。CSPE 备线仍被 X3 卡死，不影响主线。
...
| 3 | \(u_{ik}\)、\(G_k\)、item support、IRT | semantic item 粒度未定 | 全部悬空；且它依赖数据卡先完成 |
...
| **E-5** | **pilot shortcut 与 Gate A 判据存在内部不一致** | §4.2 先导只跑 1 个外层 cell，而 Gate A ① 要求被试簇 bootstrap CI。**必须写明 Gate A 的统计被试群体是"该外层训练折内、经内层 cross-fitting 得到 OOF 分数的训练被试"，而非 2–3 名留出被试**；否则该 CI 无法计算 | 在 §7.2 补一句统计群体定义；若采用留出被试口径，先导必须跑 \(\ge 3\) 个外层 cell |
...
4. **\(A\) 已由 §4.7 裁定并关闭 X2**，故先导 Days 3–5 的前置只剩 **semantic item 粒度裁定**一项；A1 前端就绪且 §4.7.5 的四项准入自检通过后，Gate A 即可启动（ANMA 原始定义已由 §6.15 关闭，backbone 已由 §4.7 关闭）。

**一句话判定（v3.4 修订）**：本文档已经完备到「可以据此直接写训练代码」的程度；离「可以据此跑出 Gate A」只剩 **semantic item 粒度**一个裁定项与 **E-5** 一个口径补写（E-1 至 E-4 由附录 F 裁定，**X1 由 §6.15 关闭，X2 由 §4.7 关闭，E-6 由 §4.6 回填**）。ANMA-orig 的测量模块、direct \(u^{+}\) 路径、A1 特征前端与全部诊断代码均属附录 E.1 桶 1，**今天即可开工**。
...
| 附录 E.2 | E-1 至 E-4 状态改为**已裁定**；E-6 已由 v3.4 回填 §4.6；仅 E-5 仍未决 |
