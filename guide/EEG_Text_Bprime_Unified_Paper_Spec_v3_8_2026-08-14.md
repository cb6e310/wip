...
| ROAMM `ds007629 v1.3.0` | 44 | 所有人阅读同 5 篇文章 | **强制外部复现 panel** | 先过 `S0_ROAMM_ADMISSION`；冻结 ZuCo route 后运行；不得反向调参 |
...
```text
先修正 S0_TEXT_ENCODER（256 + 真实 config manifest cache binding）
└─ 通过 → S0_ROAMM_ADMISSION（只做结构/来源/支持/候选上界审计，无训练）
   ├─ PASS → 扩展 outer/inner/candidate 工具到 ZuCo2 + ROAMM 两个独立 panel
   └─ STRUCTURAL NO-GO（且尚未看任何 outcome）→ 才允许审计 DERCo
```

ROAMM 未准入不阻止 ZuCo 的 Gate A/B 探索，但“论文完成”与“跨数据集复现”必须等待一个非同源第二 panel 完成。不得靠降低 item 支持门、跨 article 借 candidate、翻译文本或改 encoder 来强行通过第二数据集。
...
| 现阶段状态 | source-slot join、outer split 与全局 support audit 已完成；text encoder 因 256/config-hash 问题重开，inner split、candidate、A1 real admission 与 leakage 仍未完成 | 官方结构已人工核到 44×5 raw/synced、5 articles、487/445 句；尚未生成仓库数据卡、真实 schema/unit/support/near-duplicate/candidate ledger，故 `experiment_ready=false` |
