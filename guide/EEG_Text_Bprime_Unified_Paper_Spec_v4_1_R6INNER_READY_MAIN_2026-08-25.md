# EEG–Text 跨被试对齐小论文统一规格（v4.1-R6INNER-READY-MAIN / R6-EQALIGN-MSP）

**唯一研究主线 EQ-ANMA：受约束、可回退、fit-only 的对齐训练控制器**

> 版本：**v4.1-R6INNER-READY-MAIN（R6_EQALIGN_MSP）**，2026-08-25
...
> 对齐基线：`main@309b70163707c145b9e92a38b41a3ff92cf0f510`（R6 6×3/3×3 split reconciliation 已提交）
> 状态：**R6 split reconciliation 已完成；inner-selection 仅允许进入合同前置核验，非 estimand 变更**。primary estimand 的科学内容与 v4.0 一致；本版把仓库事实推进到 R6_INNER_SELECTION，但不把缺失的真实训练 recipe 或 OOF probe 接口用合成实现替代。
> 本版允许 Codex 在所有真实输入合同齐备时读取 outer-train/inner-train/inner-validation 并生成 inner-only metric；仍绝不允许读取 outer-test 或 calibration，也不允许产生 paper-level outcome。

> **权威性声明（R6INNER-READY-MAIN）**：本文继承 R4ALIGN、MAIN-APPROVED、R6FREEZE、R6IMPLEMENT-READY 与已完成的 R6 split reconciliation；R6 author freeze 已在 `main@125d72c9...` 完成，arm/controller/scope/ledger synthetic contract surfaces 已在 `main@a4a3d3c...` 完成，独立 6×3/3×3 split surfaces 已在 `main@309b701...` 完成。上传的 v4.1 原文、R4ALIGN、MAIN-APPROVED、R6FREEZE、R6IMPLEMENT-READY 与 R6SPLIT-RECONCILE 文件保持历史不可变。本修订以 `PROJECT_STATE.yaml`、`HANDOFF.md`、`TASKS.yaml`、已提交 artifacts、当前 `main` tree 和远程 refs 事实为来源。

> **当前状态覆盖（优先于本文的历史 R4ALIGN/R6FREEZE/implementation/split 段落）**：`main@309b701...` 是当前唯一未来工作基线；R4 对齐、main activation、R6 author freeze、`R6_IMPLEMENT_ARMS_AND_TESTS` 与 `R6_SPLIT_RECONCILIATION` 均已完成。本文中的历史提交步骤仅作为不可变记录，不是当前行动指令。当前任务是 `R6_INNER_SELECTION` 的合同前置核验：只可使用新 namespaced 6×3/3×3 split，禁止旧 6×5；在真实训练前必须核验 alignment training recipe 与 OOF probe→2PL 接口是否已被作者冻结。若任一具体值或接口缺失，Codex 必须停止并记录 `BLOCKED_R6_EXECUTION_CONTRACT_INCOMPLETE`，不得从 synthetic smoke 推断。
...
### 0.5 当前可执行任务（R6INNER-READY-MAIN；覆盖历史 split 段落）

`R4_STATE_RECONCILIATION`、R4→`main` 激活和 `R6_AUTHOR_FREEZE_ON_MAIN` 均已完成。
split reconciliation 已完成；当前唯一任务是先做 `R6_INNER_SELECTION` 合同前置核验：

`R6_SPLIT_RECONCILIATION` 已在 `main@309b70163707c145b9e92a38b41a3ff92cf0f510`
生成独立的 R6 `K_S_out=6, K_T_out=3` outer split 与每个 outer cell 的 task-global
`3×3` inner split，并把它们的 physical/canonical SHA256 写入 split contract。
现有旧 artifact `01_data_protocol/splits/zuco_2_0_outer_folds.json` 与
`01_data_protocol/splits/zuco_2_0_inner_folds.json` 是 v3.13 的 `6×5` outer
历史输入，必须保持 byte-identical，不得覆盖、改名替换或让 R6 读取。

当前 `R6_INNER_SELECTION` 允许读取 outer-train/inner-train/inner-validation 的真实
输入并产生 inner-only metric，**但必须先确认 I.2 的 recipe 与 OOF-probe 合同已经
冻结**；缺失时只记录 blocker，不读取 EEG/text、不训练、不生成指标。outer-test、
calibration、Gate、A3、ROAMM、paper-level outcome 仍被禁止；仍继续在 `main` 工作，
不得新建研究分支。历史 split reconciliation 的禁止边界与验收记录保留在 H 节。
...
**R6SPLIT-RECONCILE 覆盖条款（优先于本节旧的 A/B/implementation 叙述）**：R4
状态对齐、作者审阅、main activation、author freeze 和 arm/contract implementation
均已完成。`main@a4a3d3c...` 已绑定 R6 implementation contract；不得把 R4 inner
diagnostic 当作 R6 held-out outcome。当前只允许完成 `R6_SPLIT_RECONCILIATION`：
旧 v3.13 的 6×5 split artifact 必须保持 immutable，R6 独立 6×3 split 必须先生成
并 hash；在此之前不得进入 inner selection、训练或任何 metric。
...
execution_order:
  ...
  - R6_IMPLEMENT_ARMS_AND_TESTS      # DONE; synthetic contracts/tests only
  - R6_SPLIT_RECONCILIATION           # DONE; namespaced 6x3/3x3 split surfaces
  - R6_INNER_SELECTION                # current; contract preflight, then inner-only run if fully frozen
...
current_repository_gate:
  branch: main
  head: 309b70163707c145b9e92a38b41a3ff92cf0f510
  validator_provenance_branch: research/real-sham-r4-orthogonal-inner
  current_documentation_action: R6_INNER_SELECTION
  current_scientific_task: R6_INNER_SELECTION
  current_scientific_task_status: READY_FOR_CODEX_PREFLIGHT
  completion_outcome: NOT_RUN
  main_activation: DONE
  author_approval: APPROVED_TO_CREATE_R6_AUTHOR_FREEZE
  r6_release: R6_AUTHOR_FREEZE_COMMITTED
  r6_experiment_blocked_until_split_reconciliation: false
  old_split_artifact_shape: task_local_6x5_immutable
  r6_required_split_shape: task_local_6x3_plus_inner_3x3
  r6_split_reconciliation: DONE
  r6_real_runner: ABSENT
  r6_execution_contract: INCOMPLETE_UNTIL_RECIPE_AND_OOF_CONTRACT_BOUND
  next_after_inner_selection: R6_CALIBRATION_LABELING
```
...
### G.4 完成后的状态迁移（已完成；历史记录）

Codex 已在 `main@a4a3d3c...` 完成该 implementation contract；`T-01…T-09`、
`r6_contract_selfcheck.py`、`compileall`、项目状态验证和 diff 检查的远程记录均为
PASS，且真实 EEG/outer/calibration/paper-level reads 仍为 0。本节只作为实现完成的
不可变历史记录；当前行动由 H 节的 `R6_SPLIT_RECONCILIATION` 覆盖。

## H. R6 split reconciliation contract（状态冲突修复；非 estimand 变更）

### H.1 已核实的冲突

作者冻结与实现合同要求的 R6 split 是 `K_S_out=6`、`K_T_out=3`，即每个 task
18 个 outer cells；每个 outer cell 再使用 task-global `3×3` inner cells。远程
`main@a4a3d3c007639029c8d57d4b1700cdd00587e307` 中仍存在的旧输入则是：

| 输入 | 旧 artifact 实际内容 | 物理 SHA256 | R6 处理 |
|---|---|---|---|
| `01_data_protocol/splits/zuco_2_0_outer_folds.json` | v3.13 task-local `6×5`，30 cells/task | `20aedfd56c7c5ee41a491e5b5531ef77728344808d42ba45d72d7e30250cffa6` | 只读、历史保留、R6 禁止读取 |
| `01_data_protocol/splits/zuco_2_0_inner_folds.json` | 对上述 30 cells/task 生成的 inner artifact | `0271aba0ae9627f35e281029a8a95390513c1e92c8010e8a2795171f837575d7` | 只读、历史保留、R6 禁止读取 |

因此，直接复用旧文件会把 R6 冻结的 text-fold 数从 3 改成 5，必须判为
`STATE_SPEC_CONFLICT`，不得以“同一数据集”或“旧 artifact 已通过验证”为理由
放行。

### H.2 唯一允许的修复

在 `main@a4a3d3c...` 上新增独立、可追溯的 R6 split surfaces；不得覆盖旧文件：

```text
01_data_protocol/splits/eqalign_r6_zuco_2_0_outer_folds.json
01_data_protocol/splits/eqalign_r6_zuco_2_0_inner_folds.json
04_results/audits/eqalign_r6_zuco2_inner_split_support.json
artifacts/eqalign_r6_split_contract.yaml
```

outer builder 必须复用已验证的 ZuCo source-slot join 与 numeric sentence
admission，只把 `build_joint_split` 的 `k_subject=6, k_text=3` 作为 R6 新配置；不得
从旧 6×5 artifact 合并/重标 text fold。inner builder 必须对新 R6 outer 的每个
cell 独立生成 task-global `3×3`，并保留 outer-test subject/stimulus isolation。
若需要复用 `inner_split.py`，只能增加参数化的公开入口；不得改变旧 v3.9/v3.13
默认行为或旧 artifact 校验结果。

### H.3 读取与禁止边界

- 允许：source join、summary sequence、数值叶 shape/finite/schema admission、
  semantic-item identity ledger；允许读取数据文件的元数据以构造 split。
- 禁止：加载任何 EEG 数值用于特征、加载 frozen text encoder、训练任何模型、
  计算 retrieval/R@1/Δ、读取 outer-test/calibration rows、生成 held-out output。
- `r6_real_eeg_value_reads=0`、`outer_test_reads=0`、`calibration_reads=0` 必须写入
  split contract 与 run record；历史输入 read counters 不得改变。

### H.4 必须通过的机械断言

1. 每个 task 恰好 18 个 outer cells，subject folds 为 `{0,1,2,3,4,5}`，text
   folds 为 `{0,1,2}`，所有 fold 非空；
2. 每个 task 的每个 outer cell 恰好 9 个 inner cells，且 train/validation/
   held-out-only 对 outer-train 为不重叠完备划分；
3. subject、stimulus、atomic group 不跨越对应 held-out 边界；outer-test 不进入
   inner train/validation；
4. 同顺序构建、逆序输入构建都产生 byte-identical canonical payload；
5. 新 outer/inner/support artifact 的 physical SHA256、canonical payload SHA256、
   source-code manifest 与 config hash 全部写入 `eqalign_r6_split_contract.yaml`；
6. 旧 6×5 两个 artifact 的 SHA 在运行前后逐字节一致；`TASKS.yaml`、R0–R4 formal
   artifacts 与既有 run ledger 不变。

### H.5 完成后的状态迁移

只有 H.4 全部通过时，才将 `PROJECT_STATE.yaml` 的当前任务置为
`R6_INNER_SELECTION` 的 `READY`，并把本版 SPEC、split contract 和三项新 artifact
hash 绑定。`R6_INNER_SELECTION` 仍是下一次独立 Codex 任务；本次 split reconciliation
不得顺便训练、选 gamma、选择 DIRECT 变体或写任何 inner metric。

## I. R6 inner-selection readiness contract（当前任务；只读 inner，不读 outer）

### I.1 已完成的前置事实

远程 `origin/main` 已核验为 `309b70163707c145b9e92a38b41a3ff92cf0f510`。R6 split
reconciliation 已完成：新 outer 物理 SHA 为
`445e640239cff4cbbdb9fd0a81e6cdc8d9e0e1e1a698df4235b6382fb7637794`，新 inner 物理
SHA 为 `1b609a7e4a62c66f8b1a2127a94ec4ee42793521d7accad40178eb5cde54b40d`，support
audit 物理 SHA 为 `055be044f222ae38667d5dd3da3828f1901a890f1448d8215c067c960d105d57`；
canonical hashes 与旧 6×5 文件 hashes 见 `PROJECT_STATE.yaml` 和 split contract。

### I.2 真实执行前的硬阻塞（不得猜测）

当前仓库没有真实 R6 runner、alignment/training/retrieval package。更重要的是，
author freeze 只写了“optimizer / batch / steps / early-stopping 等必须共享”，没有
给出这些量的具体冻结值；也没有把 `u_oof → amortized 2PL → model-implied Fisher → h`
的真实 OOF probe 折数、probe architecture、fit/normalization scope 与 ledger schema
绑定到可执行代码。`eq_anma_smoke.py` 与 `run_eq_anma_synthetic_benchmark.py` 是
明确标记的非论文/synthetic surfaces，任何其中的 epochs、learning rate、batch size
或 schedule 都**不能**被提升为 R6 论文 recipe。

因此，Codex 的第一动作必须是合同前置核验：若找不到一份在任何 R6 EEG/metric 产生
之前已冻结、可 hash 的 real training recipe 与 OOF-probe contract，立即停止并记录
`BLOCKED_R6_EXECUTION_CONTRACT_INCOMPLETE`；不得创建伪 metrics，不得从 smoke 代码
推断，不得读取 outer-test 或 calibration。该阻塞不是 split 问题，也不是数据不足，
而是防止事后选择训练自由度的科学完整性要求。

### I.3 若且仅若合同齐备，允许的 inner scope

1. 只读取新 namespaced R6 outer/inner split、candidate-common-support、H、text
   encoder freeze、A1 source/frontend freeze；旧 `zuco_2_0_{outer,inner}_folds.json`
   永远不得读取。
2. 每个 `(task, outer_cell, inner_cell)` 只用该 inner cell 的 `train_record_ids` 拟合
   normalizer、OOF probe、2PL/controller 与 alignment；只用同一 inner cell 的
   `validation_record_ids` 计算选择指标；`held_out_only_record_ids`、outer-test
   和 calibration rows 不得进入 fit、selection、early stopping 或任何 cache。
3. 运行 `BASE`、8 点 `DIRECT`、4 点 `EQ_ANMA`、3 实现 `EQ_SHUFFLE`，以及无条件的
   `DIRECT_MATCHED`；所有臂共享 recipe、seed set、data order、candidate list、text
   targets、compute counters。主 seed 数为 3，消融 seed 数为 2；不得选最好 seed。
4. 选择函数严格为 §15.2：先在每个 outer cell/task/seed/subject 形成
   candidate-common-support macro-subject `R@1@N=10`，再按 pooled mean 选择 gamma；
   小于 0.25 pt 的平手取较小绝对 gamma。task-wise gamma 只记录，不运行第二套 outer。
5. 输出只允许是 inner-selection artifact、hash-bound ledger、locked gamma/DIRECT
   variant/recipe 与 run record；不得写 PASS/FAIL paper outcome、outer metric、Gate、
   calibration label 或 mechanism claim。

### I.4 预期输出（路径冻结；没有真实数据时不得生成空指标）

```text
02_code/src/eqalign_r6/inner_selection.py
02_code/scripts/run_eqalign_r6_inner_selection.py
02_code/scripts/r6_inner_selection_selfcheck.py
02_code/tests/test_eqalign_r6_inner_selection.py
artifacts/eqalign_r6_inner_selection.yaml
04_results/inner_selection/eqalign_r6_inner_selection.json
04_results/inner_selection/eqalign_r6_inner_selection.md
04_results/inner_selection/eqalign_r6_inner_selection_run_ledger.jsonl.gz
runs/research/<new>_v4_1_r6_inner_selection.md
```

缺失 dataset、exact text snapshot 或任一 frozen input hash 时，输出 BLOCKED run
record 即可，不能用 synthetic data 或 smoke defaults 填入上述 metric artifact。

*文档结束。v4.1-R6INNER-READY-MAIN 基于已提交的 `main@309b701...`；本版记录 split
reconciliation 的完成事实，并把 R6 inner selection 的真实执行合同缺口显式化；未改变
R6 estimand、primary metric、arm、gamma、DIRECT、shuffle、seed、budget 或 inference
rule。*
