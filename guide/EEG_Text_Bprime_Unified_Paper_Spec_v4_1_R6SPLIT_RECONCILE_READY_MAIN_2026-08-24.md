# EEG–Text 跨被试对齐小论文统一规格（v4.1-R6SPLIT-RECONCILE-READY-MAIN / R6-EQALIGN-MSP）

**唯一研究主线 EQ-ANMA：受约束、可回退、fit-only 的对齐训练控制器**

> 版本：**v4.1-R6SPLIT-RECONCILE-READY-MAIN（R6_EQALIGN_MSP）**，2026-08-24
...
> 对齐基线：`main@a4a3d3c007639029c8d57d4b1700cdd00587e307`（R6 author freeze 与 arm/contract implementation 已提交）
> 状态：**R6 implementation committed + split-state reconciliation required，非 estimand 变更**。primary estimand 的科学内容与 v4.0 一致；本版只修正“冻结要求的 6×3 split”与仓库现有“旧 6×5 artifact”的执行入口冲突，仍不释放真实 R6 训练或 outer 实验。
> 本版不读取、不生成任何 EEG 数值、训练输出、held-out model metric、Gate 结果或 paper-level outcome。

> **权威性声明（R6SPLIT-RECONCILE-READY-MAIN）**：本文继承 R4ALIGN、MAIN-APPROVED、R6FREEZE 与已提交的 R6 implementation contract；R6 author freeze 已在 `main@125d72c9...` 完成，arm/controller/scope/ledger synthetic contract surfaces 已在 `main@a4a3d3c...` 完成。上传的 v4.1 原文、R4ALIGN、MAIN-APPROVED、R6FREEZE 与 R6IMPLEMENT-READY 文件保持历史不可变。本修订以 `PROJECT_STATE.yaml`、`HANDOFF.md`、`TASKS.yaml`、已提交 artifacts、当前 `main` tree 和远程 split 文件事实为来源。由于当前仓库仍只有旧 v3.13 的 6×5 outer/inner split artifact，而 R6 freeze 写死 6×3，Codex 必须先完成本版的 split reconciliation；在独立 R6 6×3 artifact hash 固定前，不得进入 inner selection。

> **当前状态覆盖（优先于本文的历史 R4ALIGN/R6FREEZE/implementation 段落）**：`main@a4a3d3c...` 是当前唯一未来工作基线；R4 对齐、main activation、R6 author freeze 与 `R6_IMPLEMENT_ARMS_AND_TESTS` 均已完成。本文中的历史提交步骤仅作为不可变记录，不是当前行动指令。当前任务是 `R6_SPLIT_RECONCILIATION`：在不修改旧 6×5 artifact 的前提下，生成并验证独立的 R6 6×3 outer + task-global 3×3 inner split surfaces；真实 R6 runner、训练、inner metric、outer/calibration 仍被禁止。只有 split contract 与 hashes 通过后，下一任务才是 `R6_INNER_SELECTION`。
...
### 0.5 当前可执行任务（R6SPLIT-RECONCILE-READY-MAIN）

`R4_STATE_RECONCILIATION`、R4→`main` 激活和 `R6_AUTHOR_FREEZE_ON_MAIN` 均已完成。
当前唯一任务是：

`R6_SPLIT_RECONCILIATION` — 在
`main@a4a3d3c007639029c8d57d4b1700cdd00587e307` 上生成独立的 R6
`K_S_out=6, K_T_out=3` outer split 与每个 outer cell 的 task-global `3×3`
inner split，并把它们的 physical/canonical SHA256 写入 split contract。
...
此任务只做 source-join/schema/split construction 与 deterministic/adversarial
validation；不读取 EEG 数值、不加载 text encoder、不训练、不计算 inner/outer
metric、不生成 held-out/calibration output，也不修改 R0–R4 formal artifact 或
`TASKS.yaml`。完成后下一项才是 `R6_INNER_SELECTION`；仍继续在 `main` 工作，不得新建
研究分支。
...
execution_order:
  ...
  - R6_SPLIT_RECONCILIATION           # current; new 6x3 split only
  - R6_INNER_SELECTION
...
current_repository_gate:
  branch: main
  head: a4a3d3c007639029c8d57d4b1700cdd00587e307
  validator_provenance_branch: research/real-sham-r4-orthogonal-inner
  current_documentation_action: R6_SPLIT_RECONCILIATION
  ...
  r6_experiment_blocked_until_split_reconciliation: true
  old_split_artifact_shape: task_local_6x5_immutable
  r6_required_split_shape: task_local_6x3_plus_inner_3x3
  next_after_split_reconciliation: R6_INNER_SELECTION
```
...
*文档结束。v4.1-R6SPLIT-RECONCILE-READY-MAIN 基于已提交的 `main@a4a3d3c...`；本版只修正冻结 split 与仓库旧 artifact 的状态入口冲突，未改变 R6 estimand、primary metric、arm、gamma、DIRECT、shuffle、seed、budget 或 inference rule。*
