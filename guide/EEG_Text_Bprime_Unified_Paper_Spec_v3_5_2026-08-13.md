# EEG–Text 跨被试对齐小论文统一规格（v3.4 合并版）

**主线 EQ-ANMA / 次线 CSPE：故事骨架 + 定量实验规格 + 执行合同**

> 版本：**v3.5**，2026-08-13（文件名保留 v3_4 以避免破坏既有引用）
> - **v3.5（本版）**：根据仓库 `2026-08-13_009_state_reconciliation` 的状态审计，补入一层**作者级科学冻结**，但不把需要真实数据、外部权利/污染证明或代码测试的事项提前标成完成：① A1 的 PSD 计算、native-unit 与 robust-normalization 口径冻结；② ZuCo 参考/空值处理、材料身份 join 的保守原则冻结；③ 主 semantic item 冻结为 task-local 的 released lexical content-word type，低支持率不再事后切换 cluster，而按 No-Go 处理；④ 给出确定性的 subject×stimulus 联合切分算法与 Gate-A 的 cluster population 口径；⑤ 以官方 LaBraM README 与仓库实现核实 A3 的预处理/pooled-embedding 合同，但 CO-N7、权利、通道映射和真实抽取仍是 blocker；⑥ 明确 ANMA-orig 的**科学算法定义已冻结，源码与测试尚未完成**。本版不关闭任何需要机器测试或人工事实核查的 blocker。
...
### 1.2.1 v3.5 作者级科学裁决（不替代代码/事实核查）

| 编号 | 裁决 | 作用域 | 未关闭的验证边界 |
|---|---|---|---|
| **D1** | A1 PSD 按 §4.7.1 的 raw-unit Hann/periodogram/half-open-band 公式；robust normalization 后进入模型 | A1 两种 segmentation 版本 | 128→105 continuous map 与真实数据 admission |
...
| **D4** | 材料身份只能由 source-slot/BIDS key 证明；文本 hash 只作一致性校验，不能当 stimulus identity | 所有 candidate/split | ZuCo duplicate-slot 一对一 join、TMNRED license/event policy |
| **D5** | 外层 6×5 fold 按 §4.2.1 的 deterministic balancing/hash 生成；Gate-A cluster 先 subject 内平均再 bootstrap | 所有 Gate-A 主判定 | split artifact、unit tests、真实 OOF |
| **D6** | A3 只核实到官方 preprocessing/constructor/pooling contract；仍不清 CO-N7、权利、canonical channel map、真实抽取 | T6/K7 only | 四项人工/真实数据 blocker |
| **D7** | ANMA-orig 的数学/伪代码已冻结，但源码、参数恢复、退化诊断测试未完成 | T1/T2 reference baseline | `S0_ANMA_ORIG` code/test acceptance |
...
| X2 | \(A\) 的具体选择、预训练权重、输入输出张量合同 | ~~事实~~ → **v3.4 已关闭** | **不再是 blocker**：主 \(A\) = A1、第二 \(A\) = A3；特征定义、归一化、切分版本、提取协议、张量合同与准入自检全部由 **§4.7** 给定（J27）。仅剩的事实性依赖是 §13.1 第 12–17 条的核实项，其中 A3 的**预训练语料污染核实**为一票否决（CO-N7） |
...
#### 4.2.1 v3.5 确定性联合切分算法（科学合同；实现测试仍未完成）
...
3. **cell 定义**：cell \((s,t)\) 的测试集是 subject fold \(s\) 与 text fold \(t\) 的笛卡尔交集；训练集同时排除这两个折。预处理、通道选择、归一化、semantic-item 支持统计、候选集、probe、阈值和超参全部只在该 cell 的训练集内拟合。
...
| 项 | 规格 | 标记 |
|---|---|---|
| 特征定义 | 每个时间单元一个向量：通道 × 频带的带功率（θ1/θ2/α1/α2/β1/β2/γ1/γ2 八带），ZuCo 词级发布矩阵与仓库审计均为 105 个 EEG 通道，维度 \(105\times 8=840\) | 【新·v3.5】 |
| 切分版本（主） | **ET 对齐的词级切分**（ZuCo 随数据提供），与本领域已发表工作同口径 | 【新】 |
| 切分版本（敏感性，**强制**） | **ET 无关的固定窗**：整句连续 EEG 按 1 s 窗、0.5 s 步长均匀切分，不使用任何注视信息 | 【补】 |
...
\(\lvert\mathrm{rFFT}(x\odot h)\rvert^2/(f_s\sum h^2)\)，每个半开频带 \([f_{\rm low},f_{\rm high})\) 以频率步长求和积分；**不取 dB、不做跨频带再标定**。数值单位保留为输入 release unit\(^2\)，随后只在外层训练折做 0.5/99.5% 截断与 median/IQR robust z-score；不允许隐式换算到 µV 或把单位/序列长度作为特征。该决定关闭“公式未定义”的科学缺口；真实 128→105 map 仍是事实 blocker。
...
| 项 | 规格 | 标记 |
|---|---|---|
| 权重 | LaBraM-Base 官方发布 checkpoint；仓库已记录 checkpoint bytes/hash 与 release constructor，许可证/权利仍须人工核实 | 【新·v3.5 + 核】 |
...
**v3.5 A3 核实边界**：官方仓库明确要求提供输入 channel order，并给出上述预处理口径；项目仓库的 A3 contract 已通过 checkpoint/load/pooling/shape/no-gradient 的合成检查。以上只能证明**工程合同可执行**，不能证明预训练语料排除了 ZuCo/自然阅读（CO-N7）、权利可用于投稿、EGI128→模型 canonical electrodes 的映射正确，或真实 MAT 已成功抽取。后三者仍是一票否决/停止条件。
...
\tilde w^{\rm dir}_i=
\begin{cases}
\dfrac{1}{\left|K_i\right|}\displaystyle\sum_{k\in K_i}\left(u^{+}_{ik}\right)^{\gamma}, & \displaystyle\sum_{k\in K_i}u^{+}_{ik}>0\\[2ex]
\eta\cdot\operatorname{median}_{j}\tilde w^{\rm dir}_j, & \text{否则}
\end{cases}
\qquad \eta=0.1
\]
...
**没有第二个损失项**。\(u^{\mathrm{OOF}}_{ik}\) 与 ANMA-orig 的 \(y_{ik}\)、EQ-ANMA 的软标签来自同一批 Stage-1 折外分数，在冻结 latent 上一次算完、训练期间不刷新（§6.11）。句子级聚合取均值、地板 \(\eta\)、批内归一化到平均权重 1、stop-gradient 全部与其它加权行一致（附录 F.2）——这些属 L1，必须相同。
...
w_t = masked_mean(u_plus ** GAMMA, K(batch))        # MEAN over items, same operator as F.2
w_t = where(w_t <= 0, ETA * median(w_t), w_t)       # ETA = 0.1, same floor as other rows
w   = stop_gradient(len(batch) * w_t / (w_t.sum() + EPS))
...
- **T6**：跨数据集 × 跨 backbone 复现表（K7）。**第二 backbone 固定为 A3 = LaBraM-Base 冻结提取**（§4.7.2），仅进本表、不参与任何 Gate 判定；A1 的 ET 无关固定窗版本作为同表内的第三列强制敏感性（§4.7.1）。若 A3 因预训练语料污染核实不通过（CO-N7），本表的 backbone 腿判为未满足，**不得**临时换入第三个 backbone 补位。
- **T7**：CSPE 阶梯表（§9.4）。
- **T8**：G2′ 检验表。
...
### 10.1 数据集台账【源，被试数来自 M §6】

| 数据集 | 被试数 | 被试–刺激分配 | EQ-ANMA | CSPE | 处置 |
|---|---:|---|---|---|---|
| ZuCo 1.0 | 12 | 近似平衡（同一句集） | ✅ 主 | ❌ G2′ 高风险 | 统计单位是语义单元，故 12 人可行；**与 2.0 分开 panel（J19）** |
| ZuCo 2.0 | 18 | 近似平衡 | ✅ 主 | ❌ | 与 1.0 任务范式有差异，**不得未经审计拼接** |
| ChineseEEG | 10 | 待核实【核】 | ✅ 副（跨语种验证） | ⚠️ 需先跑 G2′ | 支持 U1、U2；CSPE 只可探索 |
| ChineseEEG-2 | 4 + 8（跨条件非同一批人） | 天然不平衡但条件混杂 | ❌ 跨被试统计不可 | ⚠️ G2′ 可能通过但混杂 | 仅作条件迁移的补充分析，不进主表 |
| TMNRED | 30 | **待核实**【核】 | ✅ 副（若可下载） | ✅ 首选（若不平衡） | 唯一同时满足 \(n\ge 25\) 与潜在不平衡的候选 |
| Chisco | 3 | — | ❌ | ❌ | 被试数不足 |
| COFETT | 2【核】 | — | ❌ | ❌ | 仅作 TF-free / 跨天定性参照 |

### 10.2 数据集决策树（先导第 0 天必须走完）

```
TMNRED 可下载 且 被试级元数据完整？
├─ 是 → 跑 G2′
│        ├─ 通过 → CSPE 主数据 = TMNRED；EQ-ANMA 副数据 = TMNRED
│        └─ 不通过 → CSPE 在该数据上无意义，走 EQ-ANMA 单线
└─ 否 → EQ-ANMA 主数据 = ZuCo 1.0 与 2.0（各自独立 panel，J19）；副数据 = ChineseEEG
         且 CSPE 降为「仅当 EQ-ANMA Gate A 失败」的应急项，并须先在 ChineseEEG 上跑 G2′
```

**推论**【源】：若数据刺激平衡，应换数据集或换路线，**不得靠调参掩盖**。
...
### 10.4 v3.5 数据卡科学裁决（结构 PASS ≠ experiment-ready）

以下是本轮可以在不运行模型的前提下冻结的**分析政策**；它们不替代数据源核实、join 单元测试或真实信号 admission。

| 项 | ZuCo 1/2 主 panel | TMNRED 补充 panel |
|---|---|---|
| trial 纳入 | sentence `rawData` 必须是有效 numeric、非 placeholder；无效 sentence 不删除/填补为零，而从该 panel 的刺激池排除并记数 | 保留完整 subject 设计；`sub-23/ses-1` 的缺失 event IDs 按 source 记录排除，不插值、不补齐；若某 cell 因此无目标则标记 missing |
| item 纳入 | word group、word content、rawEEG container 均有效，至少一条 fixation 可解析；malformed fixation 行逐条丢弃；没有可用 fixation 的 item observation 缺失，不用 sentence EEG 伪造 | 以 material statement 的 task-local lexical item 为候选；事件级缺失按 source 缺失处理，不把 trial ID 当 item |
| 空句/空 item | sentence 可进入句子级诊断；若 \(K_i=\varnothing\)，沿用 \(\eta=0.1\) 地板；不得把空句静默当负例 | 同左；空 cell 只在数据卡中报告，不能静默重采样 |
| 材料身份 | `dataset|task|source_file|row_number|paragraph_id_raw|sentence_id_raw` 是唯一候选 source-slot key；文本相等只作一致性校验，不能作身份 | `dataset|stimulus_id` 由 BIDS event/material 对照表给出；重复/近重复审计后才进入候选池 |
| join 失败 | 无法证明 summary slot 与 source slot 一对一时，受影响 slot 不进 paper-level split；不按文本 hash 猜测 | event/material ID 不一致时整 cell 标记 unresolved，不作 imputation |
| session | 主判定仍按 subject cluster；session 只作诊断协变量，不可跨 subject fold | session 嵌套在 subject 内，默认不跨 subject fold；混 session 的影响须作为附录敏感性，不以 session ID 作特征 |
| 现阶段状态 | 结构性 data card PASS；material join、channel map、joint split、support 仍未验证，`experiment_ready=false` | 结构性 data card PASS；license discrepancy、incomplete cell、split/session policy 仍未核实，`experiment_ready=false`，仅补充候选 |

**强制记录**：每次排除都写明 `(dataset, task, subject, stimulus/source-slot, reason, raw reference type)`；禁止“清洗后剩余样本”而不报原始分母。上述 policy 完成后，`B_ZUCO2_REFERENCE_EXCLUSIONS` 的**科学定义**关闭，但其实现审计仍是 `S0_JOINT_SPLIT` / `S0_LEAKAGE_AUDIT` 的前置条件。
...
| 共同 | **CO-N7**【补·v3.4】 | 第二 \(A\)（A3）的预训练语料被核实为包含 ZuCo 或任何自然阅读 EEG 语料 | 联合留出被外部预训练语料破坏：该 backbone 出局、不得进入 T6；K7 的 backbone 腿按 §4.5 判为未满足，主张范围相应收缩，**不得**临时换入第三个 backbone 补位 |
...
| **A1 的频带/通道集合与切分版本**【v3.5·D1】 | 八带 × 105 词级 EEG 通道 = 840 维；PSD/单位按 §4.7.1；词级切分（主）+ 1 s 窗/0.5 s 步长固定窗（强制敏感性），两版并报 | 科学口径已冻结；map/admission 仍 Stage 0 |
...
| 内层 \(K^{\rm in}\) | 4 × 4，下调规则见 J17 | Gate 前 |
...
13. 【补·v3.5·D1】词级发布矩阵与仓库 audit 已核实为 **105 EEG channels**；八带、PSD 公式与 native-unit policy 已由 §4.7.1 冻结。仍须核对 source field/unit 在真实 MAT 上的一致性，以及 continuous raw 的 128→105 map（这两项不能靠文本资料猜测）。
14. 【补·v3.4】**LaBraM / CBraMod / REVE 的预训练语料清单是否包含 ZuCo 或任何自然阅读 EEG**——若包含，联合留出被破坏，该模型直接出局（**CO-N7**）。**此条优先级高于其性能表现**，须在任何 T6 实验之前完成。
15. 【补·v3.5·D6】LaBraM 官方 README/仓库已核实输入 channel order 要求、0.1–75 Hz、50 Hz notch、200 Hz、µV 口径，项目源码已核实 Base constructor 与 200D pooled output；仍须人工核实 checkpoint/weights/corpus rights、预训练语料 CO-N7、canonical electrode list 与 EGI128 映射覆盖率。
16. 【补·v3.4】**每个外层文本折的非训练刺激池实际规模**（经附录 F.3 的近重复剔除与长度匹配之后）。ZuCo 2.0 自然阅读仅 349 句、ZuCo 1.0 全库约 1107 句，\(N=200\) 是否可行须**逐折核算**；不足时按 \(N=100\) 为最大候选集执行并如实声明（附录 F.3 的可行性前置条件）。
17. 【补·v3.5·D6】官方 LaBraM fine-tuning/extraction code 已可得并已 vendored；仍须核对 2606.06647 的具体 adapter 是否与本文 pooled/no-repool 口径一致。不得把代码可得性写成 contamination/rights clearance。
...
2. ~~\(A\) 的输入输出张量合同~~ → **v3.4 已由 §4.7.1 / §4.7.2 给出，不再构成停止条件**；仍需的是 §13.1 第 12–16 条的核实结果（其中第 14 条为 A3 的一票否决）；
...
spec_version: v3_5_scientific_freeze_2026_08_13  # v3.4 + D1--D7: A1 PSD/data policy, semantic item, deterministic joint split, Gate-A population, A3 evidence boundary, ANMA-orig implementation boundary
...
task:
  ...
  n_way: [10, 50, 100, 200]
  candidate_pool: union_of_all_non_training_stimulus_folds   # v3 [F.3]
  candidate_length_match_tolerance: 0.25
  candidate_near_duplicate_cosine_max: 0.9
  candidate_lists_per_target: 5
  candidate_nesting: n200_list_prefix_subsets
  candidate_lists_shared_across_methods_and_seeds: true
  n200_requires_pool_ge_200_else_fallback_n100: true          # [核]
  verification:
    auroc_ratio: "1:1"
...
stage_2_eq_anma:
  item_definition: content_word_type_or_semantic_cluster
...
direct_u_plus_weighting:                     # v3.2, spec section 6.16, Gate B veto control
  ...
  mandatory_diagnostics: [weight_entropy_normalized, gini, weight_quantiles,
                          spearman_w_dir_vs_w_eq, floor_hit_rate,
...
backbone_a:                                  # v3.4 [J27-J29], spec section 4.7, CLOSES BLOCKER X2
  ...
  A1:
    ...
    bands: 8                                  # [核] verify ZuCo band definition
    channels: 105                             # [核] verify
    feature_dim: 840                          # [核] channels * bands
    ...
    tensor_contract:
      ...
      output: "(B, d_align)"
      d_align_equals_text_side_sentence_embedding_dim: true
...
execution_order:
  - resolve_blocker_x3_first              # x1 closed in v3.1 (section 6.15); x2 closed in v3.4 (section 4.7)
  - implement_and_freeze_A1_frontend_and_pass_admission_selfcheck   # v3.4, Stage 0
  - verify_A3_pretraining_corpus_before_any_t6_run                  # v3.4, CO-N7 is a hard veto
  - run_g0_and_leakage_audit
...
**结论（v3.5 修订）：科学规格层已完备，但先导仍受真实数据与机器验证阻断。** A1 PSD、reference policy、semantic item、joint-fold 生成算法与 E-5 population 已在本版冻结；这不等于 split 文件、support ledger、A1 admission 或 ANMA-orig tests 已通过。

v3.1 关闭了 X1（ANMA 原始定义，§6.15），**v3.4 关闭了 X2（backbone \(A\)，§4.7）**，v3.5 再关闭上述五项**科学口径**。Gate A 的实际前置依赖现在是：ZuCo source-slot join、reference exclusion ledger、A1 continuous channel map/real admission、joint split artifact/tests、semantic support audit、ANMA-orig/direct path code tests，以及 A3 仅在 T6 需要的 CO-N7/rights/map/real extraction。CSPE 备线仍被 X3 卡死，不影响主线。
...
| 9【v3.4 新增】 | **A1 特征前端 + 两个切分版本 + §4.7.5 的四项准入自检**：实现确定性带功率前端、折内归一化、词级与 ET 无关固定窗两版切分、张量合同单元测试，并在冻结表征上跑 A-A1–A-A4 | \(A\) 的张量合同 + 准入证据（进 T5） | **不需要任何外部权重、不需要预训练** |
| 10【v3.4 新增】 | **1 次真实对齐训练的单位成本实测**（回填 §4.6，判定是否触发削减序） | 预算实测数字 | 依赖第 9 项 |

第 5、第 6 与第 9 项是**最高性价比的早期动作**：\(R_1\) 可能在一周内直接杀死或极大压缩整个课题的可宣称空间；参数恢复模拟可在零 EEG 成本下提前暴露 Gate B 的识别性风险。

**桶 2：被 blocker 卡死（不得开跑）**

| # | 被卡任务 | 卡在哪个 blocker | 后果 |
|---|---|---|---|
| 1 | ~~冻结 latent、三臂 probe、\(u^{\rm OOF}\)、\(\delta\) 校准、Gate A 全部~~ | ~~\(A\) 未定（X2）~~ → **v3.4 已解除** | 主 \(A\)=A1 已裁定并给出完整规格（§4.7），本项**移入桶 1**；仅剩 semantic item 粒度一个前置 |
...
4. **\(A\) 已由 §4.7 裁定并关闭 X2，v3.5 又冻结 item/fold/E-5 的科学口径**；先导 Days 3–5 的前置不再是作者选择，而是 source join、真实 map、support ledger、A1 admission 与代码测试。Gate A 只有在这些验证通过后才可启动。

**一句话判定（v3.5）**：规格已经足以写实现，但**不能**把“可编码”写成“可跑 Gate A”。A1/A3 的工程合同、ANMA-orig 的科学算法、H、item、fold 与 Gate-A population 均已具体化；剩余事项按本版 D1–D7 的验证边界逐项完成后，才允许进入 Stage 1。
...
\tilde w_i=
\begin{cases}
\dfrac{1}{\left|K_i^{G}\right|}\displaystyle\sum_{k\in K_i^{G}} I_{ik}\,g\!\left(u^{\rm OOF}_{ik}\right), & K_i^{G}\neq\varnothing\\[2ex]
\eta\cdot\operatorname{median}_{j}\tilde w_j, & K_i^{G}=\varnothing
\end{cases}
\qquad \eta=0.1
\]
...
**地板 \(\eta\) 的作用与消融**：\(K_i^{G}=\varnothing\) 的句子若权重取 0，则该方法同时变成了数据选择方法，T1 各行的有效训练集规模不再相同，Gate B 的归因被污染。故主方法保留地板；**"无地板（硬丢弃）"作为 T3 的一个预注册消融**，并必报触及地板的句子占比。

### F.3 E-3 裁定：N-way 候选集构造

| 项 | 规格 |
|---|---|
| 负例来源 | **当前外层训练集之外的全部刺激的并集**（即所有非训练文本折），而非仅当前留出文本折 |
| 理由 | 保证负例对模型全部未见，同时把可用池扩大到约 \(4/5\) 的语料，使 \(N=200\) 在 ZuCo 级语料上可行 |
| 长度匹配 | 负例的 token 长度须落在目标长度的 \(\pm 25\%\) 内；不足时取最近邻补齐。**这是吸收 F.1 第 3 条长度 shortcut 的主要机制** |
| 近重复排除 | 与目标的冻结文本嵌入余弦 \(>0.9\) 的句子不得作负例（预注册阈值），避免不可解样本 |
| \(H\) 交叠排除 | 出现在该 trial 的 \(H^{\rm full}\) 中的句子不得作负例（保持 \(R_1\) 口径干净） |
| 嵌套结构 | 一次性构造 \(N=200\) 的列表，\(N=10/50/100\) 取其**前缀子集**，使 F5 的 N-way 曲线在同一批负例上单调可比 |
| 重复抽样 | 每个目标抽 \(L=5\) 组独立负例列表，指标取 5 组均值，降低候选集抽样方差 |
| 固定与复用 | 候选集清单按 (outer fold, seed-independent) 一次生成、落盘、**跨全部方法行与全部 seed 复用**；否则 T1 的配对差值不成立 |
| 可行性前置【核】 | \(N=200\) 要求非训练刺激池 \(\ge 200\) 个唯一句子。**数据卡必须核实每个外层 fold 的实际池规模**；若不足，按 \(N=100\) 为最大候选集执行，并在论文中如实声明，**不得**通过放宽"负例不得来自训练刺激"来凑数 |
...
|---|---|
| 正例 | 每个 trial 一个真配对 |
| 负例 | 与 F.3 同池、同长度匹配、同近重复排除规则 |
| AUROC 口径 | 1 正 : 1 负 |
| AUPRC 口径 | **固定患病率 \(1/50\)**（1 正 : 49 负），与主候选集规模 \(N=50\) 对齐；不得使用随方法/数据集浮动的患病率，否则 AUPRC 跨行不可比 |
