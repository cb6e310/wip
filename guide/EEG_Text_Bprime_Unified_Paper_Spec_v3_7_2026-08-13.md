# EEG–Text 跨被试对齐小论文统一规格（v3.7 科学冻结与执行冲突修订版）
...
| **D5** | 外层 6×5 fold 按 §4.2.1 的 deterministic balancing/hash 生成；Gate-A cluster 先 subject 内平均再 bootstrap | 所有 Gate-A 主判定 | outer artifact/tests 已完成；inner artifact 与真实 OOF 仍未完成 |
...
| **D10** | 冻结文本侧为 `sentence-transformers/all-MiniLM-L6-v2` revision `1110a243fdf4706b3f48f1d95db1a4f5529b4d41`；attention-mask mean pooling、L2 normalization、eval/no-grad、384D，sentence/item/H/near-duplicate 共用同一实现 | 所有文本嵌入、\(c^{\rm sent}\)、item amortization、H 与候选去重 | 首次真实运行须记录本地文件 SHA256、tokenizer 配置与确定性 smoke；不得静默跟随 `main` |
| **D11** | 数据集/任务 panel 独立准入、独立候选、独立泄漏审计。TMNRED 是补充 panel，其 lexical/license/session/split blocker 不得阻塞 ZuCo 2.0 的候选、泄漏审计、A1 admission 或 Stage-1 | 最低论文包与 J15 | 跨数据集主张仍须至少两个非同源数据集；TMNRED 未通过时只删相应 panel/主张，不放宽 ZuCo 协议 |
...
| **D13** | 每个 outer cell 的 inner split 必须只在 outer-train 上按 §4.2.1 的同型确定性算法生成；默认 4×4，触发 J17 才整体降为 3×3。outer artifact 完成不等于 inner cross-fitting 完成 | Stage-1 OOF、内层超参选择、direct 最强变体选择 | inner artifacts/tests 尚未实现；任何 test subject/stimulus 进入 inner 即整 cell 作废 |
...
#### 4.2.1 v3.7 确定性联合切分算法（outer 已完成；inner 仍未实现）
...
6. **inner cross-fitting（v3.7·D13）**：每个 outer cell 独立在其 outer-train records 上重跑同型分配，不得复用 outer-test 的计数或 ID。inner subject folds 先按该 outer-train 内有效 trial 数降序、被试 ID 平手，round-robin 写入 4 桶；inner text folds保持 `dataset/task/document/paragraph` group 原子性，按该 outer-train 内有效 stimulus 数降序，并以 `SHA256("20260813|outer_cell_id|inner|group_id")` 破平后贪心写入 4 桶。若 J17 任一触发项成立，则 subject/text 两轴同时降为 3×3，不能只降一轴。产物必须列出每个 inner train/validation 的 subject/stimulus/record ID、触发或未触发降级的证据、SHA256，并断言与 outer-test 的 subject、stimulus、record 三重交集均为空。**现有 outer 6×5 artifact 不满足本条；未生成 inner artifact 前不得运行 Stage-1 OOF 或任何内层选择。**
...
| tokenizer / 截断 | 使用该 revision 自带 tokenizer/config；padding 不进入均值；模型卡上限内截断，实际 token 数与是否截断必须落盘。本文 \(H^{\rm full}\) 的 64-token 上限先于模型截断 |
...
| 缓存 | 以 `(revision, tokenizer_config_hash, exact_utf8_text_sha256, pooling, normalize)` 为 key；embedding cache 按输入文本与配置 hash 绑定，跨方法/seed 只读复用 |
| 准入证据 | 记录模型文件 SHA256、tokenizer/config SHA256、384D/L2-norm/no-grad/determinism smoke、同文本 byte-identical 重跑；失败则候选、H、对齐与测量模块全部不得启动 |
...
### 10.4 v3.7 数据卡科学裁决（结构 PASS ≠ experiment-ready）
...
| 现阶段状态 | 结构性 data card、source-slot join、外层 joint split 与 semantic support audit 已完成；仍缺冻结文本编码器实现/哈希、inner split、逐 target 候选可行性、A1 `sentenceData.rawData` 的真实 source admission 与完整 leakage audit，故 `experiment_ready=false` | 结构性 data card PASS；license discrepancy、incomplete cell、split/session policy 仍未核实，`experiment_ready=false`，仅补充候选；**不得反向阻塞 ZuCo 主 panel** |

**强制记录**：每次排除都写明 `(dataset, task, subject, stimulus/source-slot, reason, raw reference type)`；禁止“清洗后剩余样本”而不报原始分母。`B_ZUCO2_REFERENCE_EXCLUSIONS` 的科学定义及 source-slot/joint-split 实现已关闭；剩余实现审计落在 inner split、候选可行性、A1 source admission 与 `S0_LEAKAGE_AUDIT`。按 D11，TMNRED 的任何 blocker 只约束其自身补充 panel/CSPE，不得出现在 ZuCo 任务的 `blocked_by` 中。
...
| **句子候选规模与可行性**【v3.7·D12】 | 负例仅来自当前 outer held-out text fold，逐 target 应用长度、近重复与 \(H\) 交叠过滤；主 (N=50) 要求每个纳入 target 至少 49 个合法负例，否则为 blocker。只构造该 target 可行的最大预注册 (N\in\{10,50,100,200\})，小 (N) 取前缀；不得以训练文本、放宽过滤或“统一回退 (N=100)”凑数 | 冻结文本编码器与 inner split 后、任何评测前生成逐 target feasibility ledger |
...
5. 外层 fold 清单；
6. 主指标、主 null、主候选集规模的预注册文件；现有 outer fold 已落盘，但在任何内层选参/Stage-1 前还必须生成 outer-test 隔离的 4×4（或统一 3×3）inner split artifact；
...
12. 【v3.7】冻结文本编码器的科学选择已由 §4.8 关闭；**实现与 admission 未关闭**。若 exact revision、tokenizer/model/config hash、384D pooling 合同或 CPU 确定性 smoke 缺失，候选近重复过滤、(H)、item amortization 与任何使用 (d_{align}) 的训练均须停机。
13. 【v3.7】inner split 与候选清单是 outer fold 之后的独立 artifact；不得把已有 outer 6×5 文件误写为“nested OOF 已就绪”，也不得在候选不足时由 Codex 自行更改 source/filter/N 规则。
...
spec_version: v3_7_protocol_reconciliation_2026_08_13  # D1--D14; no paper-level result inspected
...
task:
  ...
  candidate_pool: current_outer_heldout_text_fold_only        # v3.7 [D12, F.3]
  candidate_length_match_tolerance: 0.25
  candidate_near_duplicate_cosine_max: 0.9
  candidate_lists_per_target: 5
  candidate_nesting: largest_feasible_preregistered_n_then_prefix_subsets_per_target
  candidate_lists_shared_across_methods_and_seeds: true
  feasibility_is_per_target_after_all_filters: true
  n50_requires_49_legal_negatives_else_blocker: true
  n100_n200_currently_infeasible_from_raw_outer_fold_counts: true
  forbidden_candidate_fallbacks: [outer_train_text, relaxed_filters, replacement_sampling, silent_target_drop]
  verification:
    auroc_ratio: "1:1"
...
execution_order:
  - implement_and_admit_exact_revision_frozen_text_encoder          # v3.7; next single Codex task
  - generate_and_validate_inner_splits_inside_each_outer_cell
  - construct_candidate_lists_and_prove_per_target_n50_feasibility
...
| 项 | 规格 |
|---|---|
| 负例来源 | **只来自当前 outer cell 的 held-out text fold**。outer-train 的四个文本折一律禁止；不存在额外的“所有非训练文本折并集” |
| 理由 | 外层 5 折中训练占约 $4/5$、留出占约 $1/5$。只有当前留出折能同时保证 target 与全部负例对该 outer-cell 模型未见；v3.6 的“约 $4/5$”把训练/留出集合算反了 |
...
| 嵌套结构 | 对每个 target 与重复 (L)，一次性构造其**最大可行的预注册规模**，较小 (N\in\{10,50,100,200\}) 取前缀；不强求不可行的 (N=100/200) |
...
| 可行性前置【核】 | 可行性按 **target** 而非只按 fold 原始总数判定：经过长度、近重复与 (H) 交叠排除后，(N) 需要至少 (N-1) 个合法负例。现有 NR held-out folds 为 69–70 句、TSR 为 78 句（过滤前），故 (N=100/200) 不可行；主 (N=50) 必须逐 target 证明至少 49 个合法负例。若不足即 blocker，**不得**放宽 source/filter、用替换抽样或静默删除困难 target 来凑数 |
...
- 外层 split artifact 只有 outer 30 cells，代码库没有每个 outer cell 内的 inner 4×4/3×3 artifact；因此“nested OOF ready”仍为假。
