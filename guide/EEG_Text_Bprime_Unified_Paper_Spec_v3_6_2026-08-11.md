...
> 版本：**v3.6**，2026-08-13（本文件为 v3.6 正式文件名；旧 v3_4 文件名仅保留作兼容引用）
> - **v3.6（本版）**：在 v3.5 科学冻结基础上修正 A3 的准入分层：LaBraM 论文附录 D 的完整 **2534.78 小时**预训练语料清单已记录，未见 ZuCo 或自然阅读 EEG，故 **CO-N7 清除**；本项目将 A3 checkpoint 用于本地研究推理/冻结特征提取作为工作假设，权利/再分发范围改为披露项，不再作为 T6/K7 的硬 blocker。A3 仍需通过 EGI128→canonical channel map 与真实 MAT extraction admission。其他 v3.5 决策不变。
...
spec_version: v3_6_scientific_freeze_2026_08_13  # v3.4 + D1--D7 + A3 CO-N7 clearance/local-extraction rights scope
...
**结论（v3.6 修订）：科学规格层已完备，但先导仍受真实数据与机器验证阻断。** A1 PSD、reference policy、semantic item、joint-fold 生成算法与 E-5 population 已在本版冻结；A3 的 CO-N7 已按论文完整语料清单清除，权利/再分发范围不作为本地 T6 extraction 硬门槛；这不等于 split 文件、support ledger、A1 admission、A3 channel map/real extraction 或 ANMA-orig tests 已通过。

v3.1 关闭了 X1（ANMA 原始定义，§6.15），**v3.4 关闭了 X2（backbone \(A\)，§4.7）**，v3.5 再关闭上述五项**科学口径**，v3.6 清除 A3 的 CO-N7。Gate A 的实际前置依赖现在是：ZuCo source-slot join、reference exclusion ledger、A1 continuous channel map/real admission、joint split artifact/tests、semantic support audit、ANMA-orig/direct path code tests，以及 A3 仅在 T6 需要的 canonical map/real extraction；权利/再分发范围不再是本地 T6 extraction 的硬 blocker。CSPE 备线仍被 X3 卡死，不影响主线。
...
**一句话判定（v3.6）**：规格已经足以写实现，但**不能**把“可编码”写成“可跑 Gate A”。A1/A3 的工程合同、ANMA-orig 的科学算法、H、item、fold 与 Gate-A population 均已具体化；A3 的 CO-N7 已清除，权利/再分发范围不阻塞本地 T6 extraction；剩余 source join、真实 map、support ledger、A1/A3 admission 与代码测试逐项完成后，才允许进入 Stage 1。
