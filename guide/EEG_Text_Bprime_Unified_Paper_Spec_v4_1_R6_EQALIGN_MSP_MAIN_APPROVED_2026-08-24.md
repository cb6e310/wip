# EEG–Text 跨被试对齐小论文统一规格（v4.1-MAIN-APPROVED / R6-EQALIGN-MSP）

**唯一研究主线 EQ-ANMA：受约束、可回退、fit-only 的对齐训练控制器**

> 版本：**v4.1-MAIN-APPROVED（R6_EQALIGN_MSP）**，2026-08-24
...
> 对齐基线：`research/real-sham-r4-orthogonal-inner@e80862e943b9fbff7f5788dc109eefbf2c27a476`，待 fast-forward 到 `main`
> 状态：**protocol economization + repository reconciliation + author approval，非 estimand 变更**。primary estimand 的科学内容与 v4.0 一致；本版记录作者已批准将 R4 历史线快进并入 `main`，并在 `main` 上创建新的 R6 author freeze。
> 本版不读取、不生成任何 EEG 数值、训练输出、held-out model metric、Gate 结果或 paper-level outcome。

> **权威性声明（MAIN-APPROVED）**：本文继承 R4ALIGN 规格，并记录作者批准的 main 激活迁移；上传的 v4.1 原文和 R4ALIGN 文件保持历史不可变。本修订以 `PROJECT_STATE.yaml`、`HANDOFF.md`、`TASKS.yaml` 和正式诊断 artifact 为事实来源。若 fast-forward 前置条件不成立，先停止并报 `STATE_SPEC_CONFLICT`；不得猜测、rebase 或强制覆盖 `main`。
...
当前唯一推荐任务不是 `S0_R6_HISTORICAL_LOCK_LIGHT`，而是：

`R4_STATE_RECONCILIATION` — 将项目记忆入口、活动规格引用和分支状态对齐到
`research/real-sham-r4-orthogonal-inner@954cecd5d8885bb274dd4cde97db6255bd9cf54d`。

该任务的完成条件是文档与状态检查通过；它不产生新的 EEG 数值，不改变任何
R0–R4 formal artifact，不释放 R6。完成后状态应为
`ALIGNED_R4_BRANCH_LOCAL / AUTHOR_REVIEW_ONLY`，而不是 `R6_READY`。

作者审阅后若决定进入 R6，必须新建一个以 R4 HEAD 为 parent 的 author freeze，
明确写出 R6 的新 branch、base commit、数据消费范围和允许的第一项任务；不得
复用 v4.1 原先假定的 v3.13/main 起点。
...
**R4ALIGN 覆盖条款（优先于本节旧的 A/B 默认叙述）**：在当前仓库锚点，
Stage 0 的第一步必须是 `R4_STATE_RECONCILIATION`，不是重新扫描并假定 v3.13
或 `main`。它只核对 HEAD、活动分支、`PROJECT_STATE.yaml`、`HANDOFF.md`、
`TASKS.yaml`、`AI_START_HERE.md` 与 v3.25/R4 formal outputs，并把结果记录为
`CASE_A_BRANCH_LOCAL_R4`。因为项目状态明确要求 author review，完成这一步不得
把 `R6_DESIGN_FREEZE` 标为 READY，也不得生成 R6 input hash、跑单位成本训练或
进入 13.1 的第 2–4 项。

只有作者在状态文件中明确批准 R6 后，才可在一个新的 R6 branch/base 上执行
下列 13.1–13.2；届时历史锁必须同时引用 R4 的分支与四个 formal SHA，且不
得把 R4 的 inner diagnostic 当作 R6 的 held-out outcome。
...
execution_order:
  - R4_STATE_RECONCILIATION          # current task; documentation only
  - S0_R6_HISTORICAL_LOCK_LIGHT      # author-review release required first
  - R6_DESIGN_FREEZE
  - R6_IMPLEMENT_ARMS_AND_TESTS
  - R6_INNER_SELECTION
...
current_repository_gate:
  ...
  current_documentation_action: R4_STATE_RECONCILIATION
  ...
  r6_release: AUTHOR_REVIEW_REQUIRED
  r6_tasks_blocked_until_author_review: true
```

---

## 附录 D：下一项任务指令 —— `R4_STATE_RECONCILIATION`

> 本附录覆盖上传 v4.1 的旧 `S0_R6_HISTORICAL_LOCK_LIGHT` 指令。原因是仓库已经有真实的 R0–R4 branch-local lineage，且 R4 状态明确要求 author review；不能再把旧的 v3.13/main 假设当作当前入口。

本任务是**文档与项目记忆对齐**，不是研究运行。它不得产生 EEG 数值、训练输出、held-out metric、Gate、R6 input hash 或 paper-level outcome。工作分支必须是
...
作者明确批准后，另开新的 R6 author-freeze 任务；不得把本任务的完成自动解释为
`R6_READY` 或 `LOCKED_CASE_A/B`。
...
*文档结束。v4.1-MAIN-APPROVED 未读取、未生成任何 EEG 数值、训练输出、held-out model metric、Gate 结果或 paper-level outcome；未修改任何已冻结的历史 artifact；未改写任何历史结论。它只记录 R4→main 的批准迁移和 R6 author-freeze 的下一步。*
