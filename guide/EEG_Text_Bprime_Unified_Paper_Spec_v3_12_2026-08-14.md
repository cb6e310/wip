# EEG–Text 跨被试对齐小论文统一规格（v3.12 common-support 准入与泄漏审计冻结版）
...
spec_version: v3_12_common_support_admission_leakage_freeze_2026_08_14  # D1--D31; no EEG/paper-level result inspected
...
> **v3.12 更新提示**：E.1–E.3 保留 v3.7 当时的 readiness 历史，不再是当前任务指令。text encoder、inner split、v3.10 base candidates 与 v3.11 common-support view 已准入；当前只执行附录 P.3 的 `S0_LEAKAGE_AUDIT`。其他数学定义和 No-Go 原则仍有效。
...
> **v3.12 历史提示**：M.1–M.4 记录 v3.9 的排序及已完成 inner-split 指令。D19/D20 的 ZuCo-first/ROAMM-deferred 裁决仍有效，但不得重复执行 M.4；附录 N.3/O.3 也已完成，当前只执行附录 P.3。
...
> **v3.12 历史提示**：N.1–N.3 已由提交 `711340d` 完成。其 N=50 审计规则与 `STRUCTURAL_NO_GO_N50` 结论作为 deviation evidence 永久保留，但 N.3/O.3 不得重跑；当前唯一指令见附录 P.3。
...
以下指令已由 `e42b3bf` 完成，只作历史记录，不得再次交给 Codex；当前唯一指令见附录 P.3。
...
### P.3 唯一推荐的下一项 Codex 任务：`S0_LEAKAGE_AUDIT`

把下面整段连同 v3.12 ZIP 交给 Codex。附录 K–O 的旧 next-task 指令均已完成或失效，不得重复执行。
