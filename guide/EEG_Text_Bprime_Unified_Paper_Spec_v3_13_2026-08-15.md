# EEG–Text 跨被试对齐小论文统一规格（v3.13 泄漏准入与 A1 真实源冻结版）

**唯一研究主线 EQ-ANMA：故事骨架 + 定量实验规格 + 执行合同**

> 版本：**v3.13**，2026-08-15
> - **v3.13（本版，优先于下文全部历史条款）**：独立审查并准入提交 `d9dfe51442155fbd3854d223916c519a7757fff1` 的 `S0_LEAKAGE_AUDIT=DONE/PASS_PRE_RUN_V1_V5`。V1–V4 在不可变真实协议 artifacts 上通过，V5 仅为可执行的 `PASS_PRE_RUN_CONTRACT`；任何未来 EEG/训练 run 仍须携带自己的 fit/selection/test/calibration ledger 重新准入。本版在读取 EEG outcome、训练或 Gate 结果前修正一个可识别性冲突：A1 的逐 epoch Hann-periodogram bandpower 只读取**去均值并乘 Hann 后的 rFFT magnitude**。若在这个实际分析谱上只改 phase，840D features 恒等；若先对原 epoch 改 phase 再乘 Hann，窗函数又会改变分析谱 magnitude，已不是“只改 A1 不使用的 phase”的匹配 null。因此旧 phase-only 条款没有唯一且可识别的 A1 语义，不能作为强 sham、`u_min` 成员或 Gate No-Go；它改为实际分析谱的相位不变性诊断。D32–D34 与附录 Q 冻结替代的 A1 强 sham 家族，并把真实 MAT 源准入拆成独立且唯一下一任务 `S0_A1_SOURCE_ADMISSION`。该任务只核真实源的字段、采样率、105 通道顺序、单位证据、有限值、identity/coverage 与确定性 feature smoke；不训练、不跑 A-A1–A-A4、不产生 held-out metric。`S0_A1_ADMISSION` 在源准入通过前保持 BLOCKED，ROAMM 继续 deferred。
> - **v3.12（本版，优先于下文全部历史条款）**：审查并准入提交 `e42b3bf5233c0226dbaf225fc1eeab0c46acc331` 的 `S0_CANDIDATE_COMMON_SUPPORT=DONE/PASS_N10_COMMON_SUPPORT`。17,061/18,475 个 candidate-common-support instances、全部 1,414 个显式排除、五组 N=10 前缀、paired 1:1/1:9 与 `training_records_removed=0` 均通过真实 artifact 核验；三个新 JSON 的大体积与信息重复只记为**非阻断额度/存储效率注记**，既已正确生成便视为不可变证据，不要求压缩、删除或重构。该提交只执行了 v3.11 附录 O.3，没有完成泄漏审计。D29–D31 与附录 P 因此冻结唯一下一任务 `S0_LEAKAGE_AUDIT`：以 V1–V4 核验当前真实协议 artifacts，以 V5 冻结可执行的未来 run-ledger 准入合同；在尚无 EEG/训练 run 时只允许结论 `PASS_PRE_RUN_CONTRACT`，不得伪称训练期泄漏已实测清零。本版同时修正 A1 状态依赖：删除会自我阻塞 `S0_A1_ADMISSION` 的手工 blocker，把 common-support 与 leakage 设为其显式前置；leakage 通过后 A1 才转 READY，`S0_ALIGN_UNIT_COST` 只在 A1 admission 后放行。没有读取或生成 EEG 数值、训练输出、held-out model metric、Gate、route 或 paper-level outcome。
> - **v3.11（历史版）**：审查并准入提交 `711340dc92f177ce63620ea7b4c57264f678c5f3` 的完整候选可行性审计。`STRUCTURAL_NO_GO_N50` 是有效协议结论：18,475 个 outer/inner target instances 中只有 291 个能提供 49 个合法负例，不得继续以 \(N=50\) 作 ZuCo2 主协议。本版在任何 EEG 数值、训练或结果被读取前做作者级修订：主检索改为原预注册网格中的 \(N=10\)，且仅对每个作用域内至少有 9 个合法负例的 **candidate-common-support targets** 评分。保留 source scope、含等号 \(0.75–1.25\) 长度硬过滤、MiniLM cosine `>0.9` 与 \(H\) source-identity 排除；不回填、不放宽、不借文本。排除只定义 inner-validation/outer-test 评分人口，**不删除训练 records**。固定 \(L=5\) 个前缀清单；paired AUROC 用同列表第一负例，AUPRC 改为同一 9 负例的 1:9（患病率 0.1）。D25–D28 与附录 O 对 ZuCo2 的主 \(N\)、人口、paired 口径与状态迁移明确覆盖 J11/J18、D12/D24、§4.1/§4.3/§4.4/§6.5/§8.1/§12.4 和附录 F.3/F.4 中的 \(N=50\) 历史条款。Gate A/B 的概念判据、主 null、两个 Holm 主对比与 EQ-ANMA/direct \(u^+\) 公平性合同不变。
> - **v3.10（历史版）**：审查并准入提交 `d4b08308f6f51e4f7ba4256719641461d38bdc68` 的 ZuCo 2.0 inner split，冻结 outer-test/inner-validation 候选审计、长度无回填硬过滤、五组 hash 排序与 N=50 失败状态迁移（D21–D24、附录 N）。该审计已由 `711340d` 完成；当前不得重复执行附录 N.3。
...
| J2 | 主 null 定义【v3.13·D32】 | **A1 主分数 = 三类可识别强 sham（trial / within-trial unit assignment / channel-block permutation）的对数似然均值；并报其 \(u^{\min}\)**。phase randomization 只作 bandpower 不变性实现检查 | 均值抗单一 sham 偶然性，\(u^{\min}\) 提供最强可识别 null 的保守界；禁止用代数恒等 null 制造结构性零 |
...
| J11 | 主指标与主候选集规模 | 建议 R@1 at \(N=100\)（明确标为"待作者确认"） | T1 首列 R@1 (N=50)，另有 N-way acc @N=200 | **v3.11 最终裁决：主比较 = candidate-common-support macro-subject R@1 at \(N=10\)**；MRR@10 为次要，ZuCo2 的 \(N=50/100/200\) 因 D25 的结构审计不运行模型指标 | N=50 的原裁决在任何 EEG outcome 前被真实 candidate feasibility 推翻；N=10 已在原冻结网格内且保留 92.35% instances，细节见 D25–D28 |
| J12 | Gate B 的结构消融判据 | 定性："至少一个核心结构消融出现可解释退化" | 定量："消融 \(a_k\)、\(b_k\)、\(q_i\) 任一造成 \(\ge 50\%\) 主增益的退化" | **采纳 B 的 50%，并【补】加绝对增益前置条件**：仅当 EQ-ANMA 相对 direct \(u^+\) 的主增益点估计 \(\ge\) 预注册最小可解释效应（1.0pt common-support R@1@10）时才用比例判据；否则回退 S 的定性判据并在论文中如实写"增益过小，结构归因不可靠" | 增益接近 0 时"50% 的增益"是无意义量（比例的分母噪声主导） |
...
| J18 | 主对比多重性 | "预先指定一个主指标 / 主 N / 主 null" | "预注册只有 2 个主对比（vs ANMA、vs direct weighting），其余 Holm 校正" | **v3.11 合并为**：主指标/主 N/主 null 各锁定一个（candidate-common-support R@1、\(N=10\)、3-sham 均值），在该唯一组合上做 **2 个**主对比，Holm 校正这 2 个；其余全部为次要对比并单独 Holm | 两者正交，S 管"指标维度"，B 管"对比维度"；D25–D28 只修订 N 与评分人口，不改变两条主对比及 Holm 纪律 |
...
### 1.2.1 v3.11 作者级科学裁决（不替代代码/事实核查）
...
| **D12** | outer train 为 4/5 文本时，合法 unseen candidate pool 是当前 held-out text fold（约 1/5），不是“约 4/5”。每个 target 先做硬排除，再按 \(\{10,50,100,200\}\) 逐 target 审计；不得放宽来源/近重复/H 规则 | sentence retrieval / paired verification | v3.10 已完成该审计并触发 N=50 No-Go；v3.11 的 operational N 与人口由 D25–D28 覆盖，不删除本条历史审计义务 |
...
| **D24** | 种子 `20260813`，每 target 用稳定 tuple 的 SHA256 排序生成 \(L=5\) 个无放回负例排列；历史网格 \(N\in\{10,50,100,200\}\) 取同一排列的 9/49/99/199 负例前缀。原 N=50 审计的 paired 规则保留为 base evidence；v3.11 operational paired 口径由 D28 改为同一 N=10 prefix 的 1:1/1:9 | 候选可复现与跨方法公平性 | N=50 No-Go 已触发并准入；不得更改 maximal ordering，当前只允许派生 N=10 common-support view |
| **D25** | 提交 `711340d` 的 `S0_CANDIDATES` 审计准入，`STRUCTURAL_NO_GO_N50` 保留为正式结构证据，而非代码失败或 EEG 结果 | ZuCo2 候选可行性与 deviation log | 739 个 source-slot、190 scopes、18,475 target instances 全保留；N=50 仅 291 个可行；三个 artifact SHA256 和 130/130 服务器 suite 记入附录 O.1 |
| **D26** | ZuCo2 主检索改为 **macro-subject R@1 @ \(N=10\)**，主人口为各自 outer-test/inner-validation scope 中 `legal_count>=9` 的 candidate-common-support targets。该选择是 text/protocol-only、无 EEG outcome 的预结果修订 | ZuCo2 T1–T6、Gate 与全部方法比较 | 准入证据预示 outer 665/739（NR 306/349，TSR 359/390）、inner 16,396/17,736（NR 7,553/8,376，TSR 8,843/9,360），总体 17,061/18,475=92.35%；下一任务只做不可变投影与机器核验 |
| **D27** | common-support 排除只影响 inner-validation 选参指标与 outer-test 评分，不从任何 train split 删除 records。所有排除 target 必须以 source-slot、scope、sequential counts 和 `LEGAL_NEGATIVES_LT_9` 显式落 ledger；论文主张限定为“有候选共同支持的句子人口” | 评分人口、coverage 与主张边界 | 必报 overall/task/scope coverage、included/excluded token-length 分布与 document/fold counts；不得宣称 all-sentence 泛化，不得用 EEG/方法得分决定 eligibility |
| **D28** | v3.10 三个候选 artifacts 作为不可变 base ledger 保留；v3.11 只从已冻结 maximal orderings 派生 N=10 common-support view。每 target 仍用 \(L=5\) 组同一 9-negative prefix 与已冻结 target position；AUROC 1:1 用第一负例，AUPRC 1:9 用同一 9 负例，患病率 0.1 | 候选确定性、paired verification 与可追溯性 | 不重跑 tokenizer/MiniLM，不重算 cosine，不修改 base files；新 view 必须绑定三个 base file 及 canonical hashes，两次派生与逆序输入 byte-identical |
| **D29** | 提交 `e42b3bf` 的 `S0_CANDIDATE_COMMON_SUPPORT` 正式准入，完成结果固定为 `PASS_N10_COMMON_SUPPORT`；v3.10 base triplet 与 v3.11 derived triplet 均改为不可变协议证据 | ZuCo2 N=10 评分人口、paired verification 与 deviation evidence | outer 665/739、inner 16,396/17,736、overall 17,061/18,475；排除 length/cosine/H = 1,402/0/12；服务器 suite 138/138。三个 derived JSON 共约 59.9 MB 且有重复信息，但内容正确、可审计，属非阻断效率问题，不再耗额度返工或删除 |
| **D30** | `S0_LEAKAGE_AUDIT` 冻结为五层：V1 subject/record isolation；V2 stimulus/source-slot/material-group isolation；V3 合法 \(H\) 边界；V4 base/derived candidates、common-support、paired provenance 与 scoring-only 边界；V5 可执行 run-ledger 的 fit/selection/calibration 作用域。当前无真实训练 run，故 V1–V4 可对真实 artifacts 判 PASS，V5 只能判 `PASS_PRE_RUN_CONTRACT` 并要求每个未来 run 重新准入 | 预处理、probe、候选、选参与测试隔离 | 不得把 schema/synthetic test 写成“真实训练无泄漏”；任何未来 run 缺 ledger、fit ID 越界、inner validation 参与拟合、outer test 参与选择或 test calibration 非零均拒绝准入 |
| **D31** | 删除 `B_A1_REAL_SOURCE_ADMISSION`：它把真实源核验写在 `S0_A1_ADMISSION` 的验收中却又阻塞同一任务，形成状态自循环。`S0_A1_ADMISSION` 的显式前置增加 `S0_CANDIDATE_COMMON_SUPPORT` 与 `S0_LEAKAGE_AUDIT`；leakage PASS 后其状态转 READY。`S0_ALIGN_UNIT_COST` 的唯一前置改为 `S0_A1_ADMISSION` | Stage-0 状态机与执行顺序 | 真实 MAT sampling/channel/unit/finite/field semantics 仍是 A1 admission 内部硬验收，不因删除 blocker 而放宽；在 A1 DONE 前不得测 real unit cost |
| **D32** | A1 只读取去均值、乘 Hann 后的 rFFT magnitude。对该**实际分析谱**只随机 phase，840D bandpower 在数值精度内恒等；若在原 epoch 上先随机 phase 再重新 Hann，则窗函数卷积会改变实际分析谱 magnitude，已混入 magnitude perturbation，不能解释为 A1 的 phase-only matched null。旧 phase 条款因此从 A1 强 sham、主均值、`u_min` 与 phase-only Gate/No-Go 中移除，改为**分析谱相位不变性诊断**：合成与真实 smoke 均在同一 windowed rFFT 上旋转 phase 后直接以 magnitude² 重算八带，要求与原特征相等（预设 `rtol=1e-5, atol=1e-7`）。A1 的三类强 sham 冻结为：同被试/会话且 source/长度匹配的 `trial_shuffle`；trial 内 EEG unit/window 对文本位置的 `within_trial_unit_assignment_shuffle`；逐 trial、逐 realization 独立确定的 105 个 channel-block permutation（每个 block 保留其 8 bands） | A1 raw/latent Stage-0 admission、Stage-1 \(u^{OOF}\)、\(u^{min}\)、Gate A | 这是无 outcome 的可识别性修正：不得把 analysis-spectrum identity null，或会额外改变 magnitude 的 pre-window perturbation，冒充 phase-only 最强-null 下界；其他 backbone 若直接保留 phase，须另行预注册适用 sham |
| **D33** | 新增 `S0_A1_SOURCE_ADMISSION`，把真实源事实核验与 A-A1–A-A4 信号/表征 pilot 分离。它是泄漏审计后的唯一 READY 任务；只读真实 release/metadata/scripts，生成 compact source contract、排除 ledger、机器审计与 deterministic feature smoke，不拟合 normalization/probe/model，不读 held-out performance。PASS 后 `S0_A1_ADMISSION` 才 READY | Stage-0 状态机与 Codex 额度 | 先用小型、可失败的源门拦截 sampling/channel/unit/finite/identity 问题，避免在未准入数据上消耗 probe/training 额度；`S0_ALIGN_UNIT_COST` 仍只在完整 A1 admission 后放行 |
| **D34** | 单位只接受 release metadata/生成脚本/数据字典的显式证据，不得从数值量级或论文中的 90 µV artifact criterion 反推。若 release 对 summary/word EEG 没有显式 amplitude label，但能证明未转换、同一预处理链和跨文件稳定 scale/order，则 A1 可记录 `release_native_amplitude_unit_unlabelled`：不作 µV/V 换算，PSD 以 native-unit² 表示，后续仅做 outer-train fit 的 robust normalization，并在论文披露单位标签缺失。若连稳定 scale/provenance 也不能证明，则 `SOURCE_SCALE_UNVERIFIED` 阻断 | A1 source contract、PSD 单位与写作边界 | 禁止按 magnitude 猜单位或静默转换；native-unlabelled 是受限准入状态，不等于声称物理单位已知 |
...
- **P1（null 家族的语义不一致；v3.13·D32 覆盖 A1 成员）**：A1 的 trial shuffle、within-trial unit assignment shuffle、channel-block permutation 破坏的是**不同**的信息子集，三者 log-lik 的算术平均不对应任何单一定义的信息量。因此 \(u^{\rm OOF}\) 只能被表述为“相对一个可识别 null 家族的便利聚合”，并强制并报**保守变体** \(u^{\min}_{ik}=\log p_{\rm real}-\max_m \log p^{(m)}_{\rm sham}\)（见 §5.3）。实际分析谱的 phase rotation 对 A1 bandpower 是恒等诊断，不属于该家族；pre-window phase randomization 又会额外改变分析谱 magnitude，同样不能冒充 phase-only 成员。
...
- **v3.11 主候选规模**：ZuCo2 与后续复现 panel 的主检索统一为 \(N=10\)；只在各 scope 内 `legal_count>=9` 的 candidate-common-support targets 上评分，\(L=5\) 个固定列表取均值。\(N=50/100/200\) 在 ZuCo2 标记 structural unavailable，不进模型表也不回填；历史网格只用于解释本次 pre-outcome deviation（D25–D28）。
...
| 主比较组合 | 唯一组合：**candidate-common-support macro-subject R@1 @ \(N=10\)，主 null = 3-sham 均值** | 【v3.11·D26，覆盖 J11 的 ZuCo2 N=50】 |
...
| 指标族 | 主指标 | 报告方式 |
|---|---|---|
| N-way retrieval | **R@1 @ \(N=10\)（主）**；MRR @ \(N=10\) 为次要；R@5@10 因 chance=0.5 不作主证据，N=50/100/200 在 ZuCo2 只报 structural unavailable | 在同一 common-support 人口上报告 macro-subject、worst-subject、task/scope coverage |
| paired verification | AUROC 1:1、AUPRC 1:9（患病率 0.1）；均由同一 N=10 prefix 派生 | 与 retrieval 同一 common-support targets；macro-subject、worst-subject、95% cluster CI |
...
【v3.13·D32/D33 覆盖】四项自检只能在 `S0_A1_SOURCE_ADMISSION=DONE/PASS_REAL_A1_SOURCE` 后运行。A-A1 对 A1 使用 D32 的三个可识别强 sham；phase randomization 只作 feature-invariance 诊断，不进入均值、\(u^{\min}\) 或失败判据。A-A2 是表征身份保真度诊断，必须在当前 outer-train subjects 内按原子 stimulus/material 分组交叉验证，使每个类别在训练/验证均有实例；它不使用 held-out subject fold，也不得把 subject classifier 当作跨被试泛化结果。A-A3 的模型选择仍只使用相应 inner train/validation，outer test 只允许最终评分。A-A4 比较对象是 frontend 的 840D raw bandpower 与 frozen-initial encoder latent，不允许把 raw 自己与自己比较。
...
**A1 强 sham（进入主分数均值，\(M=3\)，v3.13·D32 覆盖旧定义）**：
1. `trial_shuffle`：在同被试/会话内按 source mode 与长度匹配，置换 trial EEG–文本配对；
2. `within_trial_unit_assignment_shuffle`：词级版本置换同 trial 的 EEG word epochs 对文本位置的分配；固定窗版本置换同 trial 的 window features 对位置的分配；
3. `channel_block_permutation`：对每个 trial/realization 独立、确定性地置换 105 个完整 channel blocks，每个 block 内八带顺序不变。

**弱对照（单独报告，不进主分数）**：`gaussian_noise`、`all_zero`。

**A1 分析谱相位不变性诊断（强制，不是 null comparison）**：对确定性合成 epoch 与 source-admission 的真实 smoke epoch，先执行与 frontend 相同的去均值、Hann 与 rFFT，再保持 magnitude 只旋转合法 phase，并直接从旋转后的 magnitude² 按相同八带重算 features；结果必须与原特征在预设 `rtol=1e-5, atol=1e-7` 内相等。不要 inverse-rFFT 后再次乘 Hann，因为那会引入第二次窗口并改写问题。超差触发实现/数值排查；相等是数学预期，不生成 `u_oof_phase_only`。历史 `phase-only` Gate 条款对 A1 raw/latent 全部失效。
...
逐条记录：
`(subject_id, session_id, trial_id, item_id, u_oof, u_min, u_text_only, u_null, fold_id, seed)`；A1 phase-invariance 的最大绝对/相对误差只写 run card，不伪造成逐 item 的 `u`。
...
\[
\Delta_{\rm null}=\mathrm{R@1}_{N=10,\,\mathrm{CS}}(\text{real})-\frac{1}{M}\sum_{m=1}^{M}\mathrm{R@1}_{N=10,\,\mathrm{CS}}(\text{sham}^{(m)})
\]
...
`(subject_id, session_id, trial_id, item_id, u_oof, u_min, u_text_only, u_null, y_correct, p_target, n_item, fold_id, seed)`
...
| ⑤ | null 稳健【v3.13·D32】 | 对 A1 三类可识别强 sham 的均值与 \(u^{\min}\) 均成立；方向不能只由 Gaussian/zero 驱动；phase-invariance 实现检查及 A-S1/A-S2/A-S3 通过 | 【源+补】 |
...
| ④ | 结构消融有解释力 | 消融 \(a_k\)、\(b_k\)、\(q_i\) 任一造成 \(\ge 50\%\) 主增益退化；**前置条件（J12，v3.11 口径修订）**：主增益点估计 \(\ge 1.0\)pt common-support R@1@10，否则回退定性判据并如实写明增益过小 | 【新+补·D26】 |
...
2. **任务结果侧**：\(\mathrm{cost}=\mathrm{R@1}_{N=10}(A)-\mathrm{R@1}_{N=10}(A+\text{raw LEACE})\ge 1\) 个百分点，且被试簇 95% CI 不含 0。【v3.11】此处使用同一 candidate-common-support 人口与 macro-subject 口径，否则"1pt"无定义。
...
**列（v3.11，8 列）**：common-support R@1 (N=10)｜MRR (N=10)｜paired AUROC 1:1｜paired AUPRC 1:9｜macro-subject｜worst-subject｜\(\Delta_{\rm null}\)（定义见 §6.5）｜被试方向一致性 (x/n)。coverage 与 excluded-target 文本诊断在同表脚注/附表强制给出，N=50/100/200 只记 structural unavailable，不填模型数字。
...
| A1 强 EEG null【v3.13】 | trial shuffle、within-trial unit assignment shuffle、channel-block permutation | 识别真实配对 EEG 贡献（主 null）；phase randomization 另作 bandpower 不变性诊断 |
...
|---|---|---|---|---|
| **F1** ★ | 决定性 null 图【v3.13】 | 横：输入类型（真实/trial shuffle/unit-assignment shuffle/channel-block permutation/Gaussian/全零）；纵：\(u^{\rm OOF}\) 被试分布 + 右副图 \(\pi_G\)；phase-invariance 误差放独立 inset；分面：留出被试或 dataset | 真实相对可识别强 sham 右移；phase 误差在预设容差内 | 任一可识别强 sham 否定增量证据，或 phase-invariance 超差显示实现问题 |
...
**列**：linear subject probe acc（应降至 chance \(=1/n_{\rm train}\) 附近）｜nonlinear (MLP) subject probe acc｜semantic probe｜candidate-common-support R@1 (N=10)｜macro-subject｜worst-subject｜effective rank / RankMe｜latent distortion \(\mathbb E\|PZ-Z\|^{2}\)
...
| **句子候选规模与可行性**【v3.12·D25–D30】 | v3.10 的 outer/inner source scope、source-slot identity、含等号 0.75–1.25 token 长度、MiniLM `>0.9`、\(H_{full}\) source identity、seed 20260813、\(L=5\) 与无回填全部保留。`711340d` 已准入 N=50 structural No-Go。ZuCo2 主人口改为每 scope `legal_count>=9` 的 common-support targets，主 \(N=10\)；只从已冻结 maximal ordering 取 9-negative prefix，不重算 tokenizer/embedding/cosine。显式落盘所有 excluded targets 与 coverage，不删训练 records | `e42b3bf` 已生成并准入 N=10 common-support candidate view、paired 1:1/1:9 与 coverage audit；三个 v3.10 base 与三个 derived artifacts 均不可修改。EEG 前仍须通过附录 P 的 V1–V5 pre-run leakage audit |
...
| Gate B 最小可解释效应 | 1.0pt common-support R@1@10（J12 前置条件的 v3.11 口径） | Gate B 前 |
...
| CSPE G5 阈值 | \(\theta_{\min}\le 60^\circ\)；cost \(\ge 1\)pt @ common-support R@1,N=10 | 看 CSPE 下游前 |
| CSPE 删除秩基准 \(r_0\) | 由 raw LEACE 训练折确定（§9.5） | G5 后、阶梯前 |
| covariance shrinkage grid | 预注册并报告敏感性 | G5 前 |
| 主比较组合与多重性 | candidate-common-support R@1@N=10 × 3-sham 均值 × 2 个主对比 + Holm | 全实验前 |
...
16. 【v3.12·D25–D30】**逐 target 候选可行性与 common-support 投影均已完成并准入**。`711340d` 证明 N=50 对 ZuCo2 structural unavailable（18,184/18,475 instances 不足 49）；`e42b3bf` 冻结 17,061/18,475=92.35% 的 N=10 显式评分人口。base/derived artifacts 均不可变；下一步只按附录 P 核验 V1–V5，不得引入 train text、放宽过滤、回填、重写 artifact 或从训练 records 删除句子。
...
5. 外层 fold 清单；
6. 主指标、主 null、主候选集规模的预注册文件；outer、task-global 3×3 inner、v3.10 base candidates 与 v3.11 N=10 common-support scoring view 均已落盘准入，但在任何内层选参/Stage-1 前还必须完成附录 P 的 V1–V5 pre-run leakage audit；
...
13. 【v3.12】outer/inner split、v3.10 base candidates 与 v3.11 common-support view 已准入；不得把这些协议 artifacts 误写为“nested OOF 已就绪”。当前只允许执行 D30/附录 P 的 pre-run leakage audit，不得由 Codex 自行更改 source/filter/eligibility/N 规则。
...
spec_version: v3_13_leakage_admission_a1_source_freeze_2026_08_15  # D1--D34; no EEG outcome/training inspected
...
task:
  ...
  historical_n_way_grid: [10, 50, 100, 200]
  primary_n_way: 10
  primary_population: candidate_common_support_legal_count_gte_9_per_scope
  eligibility_affects: [inner_validation_scoring, outer_test_scoring]
  eligibility_does_not_affect: [training_record_retention, split_assignment, preprocessing_fit]
  ...
  candidate_nesting: n10_uses_first_9_of_frozen_v310_maximal_order
  ...
  n50_outcome_zuco2: STRUCTURAL_NO_GO_N50
  n50_n100_n200_model_results_zuco2: structurally_unavailable_do_not_run
  common_support_exclusion_reason: LEGAL_NEGATIVES_LT_9
  common_support_exclusions_must_be_ledgered: true
  forbidden_candidate_fallbacks: [wrong_scope_text, cross_fold_borrowing, nearest_neighbor_length_refill, relaxed_filters, replacement_sampling, silent_unledgered_target_drop]
  verification:
    auroc_ratio: "1:1"
    auroc_negative_source: first_negative_of_each_frozen_repeat_list
    auprc_fixed_prevalence: 0.1               # 1:9, aligned with N=10
    auprc_negative_source: same_9_negative_prefix_as_n10_no_resampling
...
frozen_decisions:
  ...
  - a1_main_null_is_mean_of_three_identifiable_matched_shams_not_gaussian
  - a1_analysis_spectrum_phase_rotation_is_an_invariance_diagnostic_not_a_sham
...
stage_1_neural_contribution:
  probes: [text_only, real_eeg, trial_shuffle, within_trial_unit_assignment_shuffle, channel_block_permutation]
  a1_phase_invariance_diagnostic:
    role: implementation_check_not_null_comparison
    location: exact_demeaned_hann_windowed_rfft_analysis_spectrum
    recomputation: rotate_phase_then_integrate_unchanged_magnitude_squared_without_inverse_transform
    rtol: 1.0e-5
    atol: 1.0e-7
    forbidden_in: [main_sham_mean, u_min, gate_a_no_go]
  ...
  score: mean_log_lik_delta_real_minus_mean_of_three_identifiable_strong_shams
  ...
  output_fields: [subject_id, session_id, trial_id, item_id, u_oof, u_min,
                  u_text_only, u_null, fold_id, seed]
  run_card_fields: [phase_invariance_max_abs_error, phase_invariance_max_rel_error]
...
gates:
  gate_a:
    ...
    - holds_for_both_mean_sham_and_u_min_over_identifiable_a1_shams
  ...
  gate_b:
    ...
    - precondition_main_gain_ge_1pt_common_support_r_at_1_n10 # v3.11 [J12,D26]
  g2_prime:
  ...
  g5:
    ...
    - raw_leace_cost_ge_1pt_common_support_r_at_1_n10_macro_subject_ci_excludes_zero
...
execution_order:
  ...
  - construct_outer_and_inner_candidate_lists_and_prove_per_target_n50_feasibility # DONE v3.10; structural No-Go admitted
  - derive_and_freeze_n10_common_support_view_from_admitted_base_artifacts # current single Codex task
...
1. 主比较是否唯一？——candidate-common-support R@1、\(N=10\)、3-sham 均值、macro-subject、2 个主对比。
2. 有没有任何阈值是在看到结果之后定的？——若有，进 deviation log。
3. 有没有同时推进两条路线？——若有，立即停一条。

## 附录 E：开工判定（readiness audit）

> **v3.13 更新提示**：E.1–E.3 保留 v3.7 当时的 readiness 历史，不再是当前任务指令。text encoder、inner split、candidates、common-support 与 leakage 已准入；当前唯一指令见附录 Q.3。其他未被 D32–D34 覆盖的数学定义和 No-Go 原则仍有效。
...
v3.1 关闭了 X1（ANMA 原始定义，§6.15），**v3.4 关闭了 X2（backbone \(A\)，§4.7）**，v3.5 冻结 item/fold/E-5 科学口径，v3.6 清除 A3 的 CO-N7，v3.7 再冻结文本侧与候选算术。Gate A 的实际前置依赖现在是：已准入文本编码器/inner split/base candidate artifacts、待冻结的 N=10 common-support view、A1 真实 source admission、完整 leakage audit 与 ANMA-orig/direct path tests。A3 的 canonical map/real extraction 只在 T6 需要；TMNRED/X3 只阻塞历史补充线，**不影响 ZuCo 主线**。
...
当前最高优先级不是跑 \(R_1\) 数字或 direct 行，而是只关闭 **N=10 candidate-common-support scoring view**。上游文本编码器、inner split 与 N=50 No-Go 审计均已完成；本任务仍只构造协议 artifact，不读取 paper-level EEG 结果，完成后才进入 leakage audit。

**桶 2：被 blocker 卡死（不得开跑）**

| # | 被卡任务 | 卡在哪个 blocker | 后果 |
|---|---|---|---|
| 1 | 冻结 latent、三臂 probe、\(u^{\rm OOF}\)、\(\delta\) 校准、Gate A 全部 | N=10 common-support view、A1 real-source admission 与 leakage audit | A1/item 科学定义虽已冻结，但 nested OOF 机器证据未齐，仍不得开跑 |
...
| **E-3**（**已裁定 → 附录 F.3**） | **N-way 候选集构造规则未定义** | 负例清单直接决定 R@1 的绝对值；v3.11 的 J11 把 common-support R@1@\(N=10\) 定为唯一主指标，且候选与刺激留出交互 | 已由 D22–D28/F.3 冻结：负例只来自相应留出 text scope，稳定 hash 排序，跨方法复用，不回填；当前只派生 N=10 common-support view |
...
| 可行性门（v3.11） | `711340d` 已完成全 target N=50 No-Go 审计。当前主 \(N=10\)，每个 outer-test/inner-validation scope 只对 `legal_count>=9` 的 candidate-common-support targets 评分。不足 9 的 target 不进指标，但必须落入显式 exclusion/coverage ledger，且仍保留其 train records。N=50/100/200 在 ZuCo2 标 structural unavailable，不回填 |
...
**v3.11 不可变投影规则**：不重跑本表的过滤链，而从 SHA256 为 `51130ffc216a1f0bf50a9eeec42136555ab98ee110f3aaa265de54c3a004115a` 的 base candidate file 中读 maximal ordering。对 eligible target 逐 repeat 取前 9 个 negative indices 和已冻结 N=10 target position；对 ineligible target 只保留排除 ledger。该 view 必须同时绑定 paired base SHA256 `bc37630ea3c6c870d4388ac0c16582f742e6751d533e3656a284304d09e3ec5c` 和 feasibility base SHA256 `8f478fddc78ccb46df2c1a75945a3f90ec89f7c58ca456172a4874bef75f7960`。
...
| AUPRC 口径（v3.11） | **固定患病率 \(1/10\)**（1 正 : 9 负）；每 repeat 取同一 N=10 排列前缀的 9 个负例。与 retrieval 使用相同 common-support targets；不得重抽 |
...
> **v3.13 历史提示**：M.1–M.4 记录 v3.9 的排序及已完成 inner-split 指令。D19/D20 的 ZuCo-first/ROAMM-deferred 裁决仍有效，但不得重复执行 M.4；附录 N.3/O.3/P.3 也已完成，当前只执行附录 Q.3。
...
> **v3.13 历史提示**：N.1–N.3 已由提交 `711340d` 完成。其 N=50 审计规则与 `STRUCTURAL_NO_GO_N50` 结论作为 deviation evidence 永久保留，但 N.3/O.3/P.3 不得重跑；当前唯一指令见附录 Q.3。
...
## 附录 O：v3.11 候选审计准入、N=10 common-support 冻结与下一项 Codex 指令

### O.1 对提交 `711340d` 的审查结论

`S0_CANDIDATES=DONE/STRUCTURAL_NO_GO_N50` 正式准入，不重开。本轮独立评审得到：

- 真实 builder 覆盖 739 个 verified source-slot sentences、10 个 outer-test scopes、180 个 inner-validation scopes 和 18,475 个 target instances；两次 build、逆序输入 build 与正式 artifacts 均 byte-identical。
- 服务器记录 focused candidate tests 15/15、affected H/text/split/source/semantic tests 51/51、完整 suite 130/130。独立评审本地重新通过 focused 15/15、formal artifact validator 和全部 92,375 个 repeat rows 的 scope、target、长度、\(H\)、stable target-position 核验。本地缺少 `torch/h5py/transformers/huggingface_hub` 导致六个无关 import errors，并有一个 TMNRED skip；不推翻服务器完整 suite 或纯 candidate 核验。
- `candidate_lists.json` SHA256 = `51130ffc216a1f0bf50a9eeec42136555ab98ee110f3aaa265de54c3a004115a`（45,056,825 bytes）；`paired_verification_pairs.json` SHA256 = `bc37630ea3c6c870d4388ac0c16582f742e6751d533e3656a284304d09e3ec5c`（25,657,465 bytes）；`zuco2_candidate_feasibility.json` SHA256 = `8f478fddc78ccb46df2c1a75945a3f90ec89f7c58ca456172a4874bef75f7960`（14,932,512 bytes）。
- N=50 只有 291/18,475 target instances 可行，18,184 个不足 49 个合法负例，因此 `STRUCTURAL_NO_GO_N50` 是真实的协议结构结论，不是代码失败或 EEG/检索结果。N=100/200 对全部 target 均不可行。
- N=10 可行 17,061/18,475=92.35%。outer：NR 306/349、TSR 359/390、合计 665/739，所有 scope coverage 最低 85.71%；inner：NR 7,553/8,376、TSR 8,843/9,360、合计 16,396/17,736，所有 scope coverage 最低 82.80%。
- 1,414 个 N=10 不可行 instances 中，1,402 个在严格长度带后已少于 9 个，0 个由 cosine 阶段从至少 9 个降至不足 9 个，12 个由 \(H\) 排除后降至不足 9 个。该分布说明 No-Go 主要来自已冻结的 ±25% raw-text token-length hard match，不能通过实现修补消除。

以下作为非阻断工程注记：pure validator 没有独立拿物理文件重算每个已记录 file hash，也没有单独重算 target position；本轮独立审查已补做并通过。三个 canonical single-line JSON 较大，但可审计且非科学错误。两点都不得重开 `S0_CANDIDATES`。

### O.2 outcome-blind 作者裁决与 deviation log

1. 审查时未读取任何 EEG value、训练输出、held-out model metric、Gate/route 结果或 ROAMM data。故本裁决是候选协议事实暴露后的 **pre-outcome author revision**，而非按模型表现改协议。
2. 原主 N=50 永久记录为结构性不可行；不得隐藏、覆写或伪装成运行失败。ZuCo2 的 N=50/100/200 不再运行模型指标。
3. 主检索改为原冻结网格已包含的 N=10；主人口是每个既有 outer-test/inner-validation scope 内 `legal_count>=9` 的 candidate-common-support targets。总体预期覆盖 92.35%，每个作用域覆盖均超过 80%。
4. 不修改 source-slot identity、outer/inner source scope、target 保留、inclusive 0.75–1.25 token-length、strict cosine `>0.9`、\(H\) source-identity 排除、seed、\(L=5\) maximal ordering 或 target position。禁止重跑 encoder、重算 cosine、回填、借文本、放宽、有放回或新随机抽样。
5. common-support eligibility 只作用于 inner-validation 选择指标和 outer-test 评分；所有训练 records 仍按原 split 保留。不可行 targets 必须以 `LEGAL_NEGATIVES_LT_9`、scope/source-slot 与已有 sequential counts 落入 exclusion ledger，不得静默消失。
6. 主指标为 macro-subject R@1@10；MRR@10 为次要。paired AUROC 1:1 使用同一 repeat N=10 list 的第一负例；AUPRC 使用同一 list 的全部九个负例，固定 1:9/prevalence 0.1，不另抽样。
7. 论文必须报告 overall/task/scope coverage、included/excluded token-length 与 document/fold counts；主张限定为“candidate-common-support sentences”，不得外推为全部句子。
8. v3.10 三个 artifacts 是不可变 base ledger。下一任务只是从已有 maximal ordering 派生 N=10 view；若精确 counts 或 base hashes 不一致，必须停止为 `STATE_SPEC_CONFLICT`。

### O.3 已完成的历史 Codex 任务：`S0_CANDIDATE_COMMON_SUPPORT`

以下指令已由 `e42b3bf` 完成，只作历史记录，不得再次交给 Codex；当前唯一指令见附录 Q.3。

```text
任务：基于 origin/main 的 711340dc92f177ce63620ea7b4c57264f678c5f3，先用提供的 v3.11 ZIP 更新规格/状态文件，然后只完成 ZuCo 2.0 NR/TSR 的 S0_CANDIDATE_COMMON_SUPPORT。该任务只从已准入 v3.10 candidate artifacts 派生 N=10 common-support 评分视图；不要重开 S0_CANDIDATES，不要重跑 tokenizer/MiniLM/embedding/cosine，不要读取 EEG，不要训练，不要运行 leakage/A1/direct u+/Stage 1/Gate/route/main experiment，也不要恢复 ROAMM。

开始前与 ZIP 导入：
1. git fetch origin；确认当前分支包含 711340dc92f177ce63620ea7b4c57264f678c5f3。如果工作树有用户修改，保留并报告，不覆盖未知修改。
2. 列出 ZIP entries，拒绝绝对路径、`..`、symlink、重复路径、大小写冲突或预期清单外文件；核对我另行提供的 ZIP SHA256。只允许导入：
   - guide/EEG_Text_Bprime_Unified_Paper_Spec_v3_11_2026-08-14.md
   - PROJECT_STATE.yaml
   - TASKS.yaml
   - HANDOFF.md
   - AI_START_HERE.md
   - runs/2026-08-14_020_v311_candidate_review_n10_common_support_freeze.md
   不要删除、移动或修改其它文件，尤其不得改三个 v3.10 base artifacts。
3. 按 AI_START_HERE.md 恢复状态，运行 scripts/check_project_state.py 与 scripts/project_status.py。预期 SPEC v3.11、S0_CANDIDATES=DONE/STRUCTURAL_NO_GO_N50、S0_CANDIDATE_COMMON_SUPPORT=READY/recommended、ROAMM 仍 deferred。任何不一致先报 STATE_SPEC_CONFLICT，不猜测修复。
4. 读取 v3.11 D25–D28、§4.1/§4.3/§4.4/§6.5/§12.4、附录 F.3/F.4 与 O；打印 PROJECT SNAPSHOT 后才编码。

不可变输入准入：
1. 物理文件 SHA256 必须精确等于：
   - 01_data_protocol/candidates/candidate_lists.json = 51130ffc216a1f0bf50a9eeec42136555ab98ee110f3aaa265de54c3a004115a
   - 01_data_protocol/candidates/paired_verification_pairs.json = bc37630ea3c6c870d4388ac0c16582f742e6751d533e3656a284304d09e3ec5c
   - 04_results/audits/zuco2_candidate_feasibility.json = 8f478fddc78ccb46df2c1a75945a3f90ec89f7c58ca456172a4874bef75f7960
2. 同时使用现有 canonical JSON 规则重算并核对 artifacts 内声明的 canonical/content hashes、互相引用与 provenance；不得只信文件里的 hash 字段。任一不匹配即 STATE_SPEC_CONFLICT，停止且不重建 base artifacts。
3. 本任务不得 import/load tokenizer、sentence-transformers、MiniLM、EEG reader 或 ROAMM path；不得重算 token length、embedding、cosine、H 或 candidate ordering。只消费 base JSON 已落盘的 identity、scope、sequential counts、legal count、五个 maximal orders 与 target position。

唯一实现范围：
1. 新增：
   - 02_code/src/data/candidate_common_support.py
   - 02_code/scripts/build_zuco2_n10_common_support.py
   - 02_code/tests/test_candidate_common_support.py
   生成：
   ...
   复用仓库 canonical JSON/SHA256 helpers；不要修改 candidates.py、现有 candidate builder/test 或任何已准入 artifact，除非纯 import 兼容确有必要且必须最小化并解释。
2. 对每个 frozen outer-test/inner-validation scope-target：eligibility 唯一定义为 base ledger `legal_count >= 9`。保留所有 target 的 deterministic ledger；不可行者 reason 固定为 LEGAL_NEGATIVES_LT_9，并原样带出 raw pool→target exclusion→length→cosine→H→legal 的 sequential counts。
3. 对每个 eligible target/repeat，N=10 清单精确取既有 maximal ordering 的前九个 negative IDs，保持既有 target position、repeat ID、scope/source-slot identity、exact-text/token-length/provenance refs；不得排序、重抽、借用、回填、有放回或改变 L=5。
4. `paired_verification_pairs_n10.json` 只能从上述 N=10 view 派生：AUROC 1:1 使用每 repeat 第一负例；AUPRC 1:9 使用同一九个负例。不得另抽或读取旧 1:49 pairs 作为候选来源；旧 pairs 只作 provenance/hash-bound base artifact。
5. common-support audit 至少给 overall、outer/inner、NR/TSR、每 scope 的 eligible/total/coverage；included/excluded 的 token-length 分布与 document/text-fold counts；逐 failure-stage 计数；base 和新 artifacts 的 file/canonical hashes；明确 `scoring_only=true`、`training_records_removed=0`、claim population = candidate-common-support sentences。
6. 精确冻结断言：
   - outer NR 306/349，TSR 359/390，总计 665/739；outer scope coverage minimum 85.71%（比较用未四舍五入值）
   - inner NR 7553/8376，TSR 8843/9360，总计 16396/17736；inner scope coverage minimum 82.80%（比较用未四舍五入值）
   - overall 17061/18475 = 92.35%；ineligible 1414
   - 从至少 9 降为不足 9 的阶段：length 1402、cosine 0、H 12
   若任何精确整数不符，停止为 STATE_SPEC_CONFLICT，不调整阈值或统计口径。
7. 对同一输入做两次临时 build，并用正序/逆序输入各做一次；三个输出都必须 canonical-byte identical。正式写入时使用原子替换，不得留下部分 artifact。

测试与边界：
1. synthetic tests 至少覆盖：base physical/canonical hash mismatch 拒绝、eligibility 的 8/9 边界、所有 excluded target 保留及固定 reason、sequential counts 原样、五 repeats、前九 prefix/无放回/target position 不变、outer/inner scope identity 不变、paired 1:1 与 1:9 精确派生、输入逆序确定性、禁止 encoder/EEG/ROAMM imports/reads、训练 records removed 恒为 0。
2. real-artifact focused test 必须核对上述全部精确 counts、每 scope coverage 下界、base bindings、新 artifacts 自校验与两次/逆序 byte identity。不得因真实文件大而只跑 synthetic。
3. 运行 focused common-support tests、原 candidate tests、受影响的 state tests、完整 unittest suite、scripts/check_project_state.py、scripts/project_status.py、git diff --check；记录精确 pass/skip/fail。环境依赖问题可以如实报告，但 real-artifact derivation、focused tests、hash/count/determinism assertions 不能跳过。

PASS 后唯一允许的状态迁移：
1. S0_CANDIDATE_COMMON_SUPPORT READY→DONE，写 completed_by_run、PASS_N10_COMMON_SUPPORT、三个新 artifacts/code/tests 的 evidence 与精确 counts/hashes。S0_CANDIDATES 保持 DONE/STRUCTURAL_NO_GO_N50，不得改写或重开。
2. 删除 B_ZUCO2_N10_COMMON_SUPPORT_NOT_FROZEN；S0_LEAKAGE_AUDIT BLOCKED→READY 并清空 blocked_reason；recommended_next_task=S0_LEAKAGE_AUDIT；last_completed_task=S0_CANDIDATE_COMMON_SUPPORT。S0_DIRECT_U_PLUS 仍按执行顺序 BLOCKED，不得在本任务实现。
3. 更新 PROJECT_STATE.yaml、TASKS.yaml、HANDOFF.md、AI_START_HERE.md，新增唯一 runs 记录；不要改 SPEC 科学规则，不要删除 v3.10 run/artifacts。运行状态检查后提交并 push。
4. 最终报告 commit SHA、变更文件、三个新 artifact SHA256、base hash复核、精确 coverage/counts、测试计数、状态迁移与下一任务；不得报告任何 EEG、训练、held-out retrieval、Gate、route 或 ROAMM 结论。

若 task-local 实现细节与现有 helper API 略有差异，在不改变上述 identity/hash/count/eligibility/scoring-only 科学合同的前提下自行做最小工程决定并继续，不要因命名或排版细枝末节阻塞。任何会改变人口、候选、过滤、训练记录或统计口径的差异则必须停止报告，不能自行宽容。
```

本次 v3.11 审查和 N=10 common-support 裁决没有读取或生成 EEG 数值、held-out model metric、训练输出、Gate 结果或 paper-level outcome。它只把已暴露的 text/protocol feasibility 事实转化为可执行、可披露的主人口与下一任务。

---

## 附录 P：v3.12 common-support 准入、V1–V5 冻结与下一项 Codex 指令

### P.1 对提交 `e42b3bf5233c0226dbaf225fc1eeab0c46acc331` 的审查结论

`S0_CANDIDATE_COMMON_SUPPORT=DONE/PASS_N10_COMMON_SUPPORT` 正式准入，不重开、不返工：

- 三个 v3.10 base artifacts 的物理/canonical/provenance 绑定全部通过；三个 v3.11 derived artifacts 能从 base 重新派生且正序、重复构建、逆序构建与正式文件 canonical-byte identical。
- exact counts 全部匹配 D26–D28：outer NR 306/349、TSR 359/390、total 665/739，minimum 60/70；inner NR 7,553/8,376、TSR 8,843/9,360、total 16,396/17,736，minimum 77/93；overall 17,061/18,475，excluded 1,414；length/cosine/\(H\) transition 1,402/0/12。
- 所有 eligible repeat 保持 base 的第一批九个 negative、target position 与 \(L=5\)；AUROC 使用第一负例，AUPRC 使用同九负例；`training_records_removed=0`。
- 提交记录 focused common-support 8/8、existing candidate 15/15、full suite 138/138。独立审查再次通过 8/8、15/15、artifact hashes、state validator 与 diff check；独立环境缺少完整 ML 依赖且无系统 `/tmp`，因此不以本地 full-suite import/tempfile 失败推翻服务器 138/138。
- `candidate_lists_n10_common_support.json` 为 19,723,713 bytes，`paired_verification_pairs_n10.json` 为 29,257,299 bytes，audit 为 10,892,493 bytes。它们确有较多重复信息，但这是非科学、非阻断的存储/额度效率问题；现已成为准入证据，禁止为压缩而删除、重写或再次耗费 Codex 额度。

### P.2 V1–V5 的精确定义

| 层 | 当前真实 artifact 必查项 | 未来 run 必查项 | 允许的当前结论 |
|---|---|---|---|
| **V1 subject/record isolation** | outer test subject/record 与 outer train 完全不交；每个 inner train/validation 均属于相应 outer train，且与 outer test subject/record 完全不交 | preprocessing、normalization、probe、sham、threshold 与 model fit 的 record IDs 必须是相应 train IDs 子集 | `PASS_REAL_ARTIFACTS` |
| **V2 stimulus/material isolation** | outer held-out text fold 的 source-slot/material group 不进入 outer train；inner-validation source-slot/material group 只来自相应 outer train 且不进入该 inner train；atomic group 不跨边界 | selection/tuning 只能读取本 inner validation；outer test stimulus/material 不得进入 fit、selection 或 calibration | `PASS_REAL_ARTIFACTS` |
| **V3 legal H boundary** | `H_full/H_empty` 禁止 current/future/target tokens、target statistics、candidate IDs/scores 与 ET；H 内容只进入登记的 probe/history 接口，candidate 侧只可读取 source-sentence identity 做硬排除 | 每个 run ledger 显式声明 H mode/input fields，并通过禁止字段断言 | `PASS_REAL_ARTIFACTS` |
| **V4 candidate/provenance/scoring boundary** | base 与 derived 六个 candidate/audit artifacts 的物理/canonical hashes、source scope、source-slot、target、prefix、target-position、paired 派生与 `scoring_only=true/training_records_removed=0` 一致 | 全部方法/seed 共用同一 frozen list；不得用结果改变 eligibility、候选或 population | `PASS_REAL_ARTIFACTS` |
| **V5 executable run-ledger admission** | 当前尚无 EEG/training run，不制造“实测无泄漏”结论 | ledger 必须列出每个 preprocessing/normalization/probe/sham/model fit 的 fit IDs、inner-validation selection reads、outer-test reads、threshold/tuning reads 与 calibration count；fit 只在合法 train，inner validation 只用于选择，outer test 只用于最终 scoring，test-time calibration 必须为 0 | `PASS_PRE_RUN_CONTRACT`，且 `future_run_admission_required=true` |

V1–V4 任一真实 artifact failure 或 V5 contract/test failure 都使 `S0_LEAKAGE_AUDIT` 不得 DONE。未来每个真实训练/评测 run 还必须携带自己的 V5 ledger 并重新通过；本次 pre-run audit 不能替代未来 run admission。

### P.3 已完成的历史 Codex 任务：`S0_LEAKAGE_AUDIT`

下面指令已由提交 `d9dfe51` 完成，只作历史记录，不得再次交给 Codex；当前唯一指令见附录 Q.3。

```text
任务：基于 origin/main 的 e42b3bf5233c0226dbaf225fc1eeab0c46acc331，先用提供的 v3.12 ZIP 更新规格/状态文件，然后只完成 ZuCo 2.0 NR/TSR 的 S0_LEAKAGE_AUDIT。目标是对现有真实协议 artifacts 执行 V1–V4，并冻结/测试未来 run-ledger 的 V5 准入合同。不要读取 EEG，不要训练，不要运行 A1 admission/direct u+/Stage 1/Gate/route/main experiment，不要恢复 ROAMM，也不要修改或压缩任何 base/common-support artifact。

开始前与 ZIP 导入：
1. git fetch origin；确认工作分支包含 e42b3bf5233c0226dbaf225fc1eeab0c46acc331。若有用户修改，保留并报告，不覆盖未知修改。
2. 列出 ZIP entries，拒绝绝对路径、..、symlink、重复路径、大小写冲突或清单外文件；核对随 ZIP 提供的 SHA256。只允许导入：
   - guide/EEG_Text_Bprime_Unified_Paper_Spec_v3_12_2026-08-14.md
   - PROJECT_STATE.yaml
   - TASKS.yaml
   - HANDOFF.md
   - AI_START_HERE.md
   - runs/2026-08-14_022_v312_common_support_review_leakage_freeze.md
3. 按 AI_START_HERE.md 恢复状态；运行 scripts/check_project_state.py 与 scripts/project_status.py。预期：SPEC v3.12，S0_CANDIDATE_COMMON_SUPPORT=DONE/PASS_N10_COMMON_SUPPORT，S0_LEAKAGE_AUDIT=READY 且唯一 recommended，S0_A1_ADMISSION 因 leakage 前置未完成而 BLOCKED，ROAMM deferred。任何不一致报 STATE_SPEC_CONFLICT，不猜测修复。
4. 读取 v3.12 D29–D31 与附录 P，打印 PROJECT SNAPSHOT 后才编码。

不可变输入与精确物理 SHA256：
- outer split: 20aedfd56c7c5ee41a491e5b5531ef77728344808d42ba45d72d7e30250cffa6
- inner split: 0271aba0ae9627f35e281029a8a95390513c1e92c8010e8a2795171f837575d7
- inner support: 536ed93758baf1e4d7c8796bc164b39f7ec86a97ac8ac6b4e65bb8e782644564
- source-slot join: eb960cd0bf2cb5016f33793813cb61fa2c77c9ce07e2037cff69b29c14c104c8
- H definition: 226f92e299633997fdb9469592f6f8a36fa6c728aa24d9a7d6cb9ded8fb2ae6b
- text encoder freeze: 35e18392a285c8d09ba84a934e31dd327a18fa1a0c10a3bd8550f090cd496494
- base candidate lists: 51130ffc216a1f0bf50a9eeec42136555ab98ee110f3aaa265de54c3a004115a
- base paired pairs: bc37630ea3c6c870d4388ac0c16582f742e6751d533e3656a284304d09e3ec5c
- base feasibility: 8f478fddc78ccb46df2c1a75945a3f90ec89f7c58ca456172a4874bef75f7960
- N10 common-support lists: b3eda1c09542344e108ce162a0f414beb54a426644db18126ee1e87e36ddf097
- N10 paired pairs: 71b6b53e5686e125d067240fd6414b833ef74b46159b130d5c6097152d722771
- N10 audit: 6dfba054d8242501808e267f91efdf080f6cbd479b617819edb4baf47554c0fc
同时复用现有 validators 重算 artifact 内 canonical/content hashes 与互相引用；不得只核文件名或声明字段。任一不匹配即停止，不重建输入。

唯一实现范围：
1. 新增：
   - 02_code/src/protocol/leakage_audit.py
   - 02_code/scripts/audit_zuco2_leakage.py
   - 02_code/tests/test_leakage_audit.py
   生成：
   - 04_results/audits/zuco2_pre_run_leakage_audit.json
   - 01_data_protocol/leakage_audit.md
   优先调用 joint_split/inner_split/H/candidate/common-support 的现有 pure validators 与 canonical helpers；不要复制 builder、encoder 或 EEG 逻辑。允许为纯 validator 复用做最小 import/API 修正，但不得改变任何已准入 artifact、人口、过滤、hash/order 或科学规则。
2. V1 对真实 outer/inner/source join 核验 subject 与 record 隔离：所有 inner IDs 是对应 outer-train 子集；inner train/validation 互斥；二者与 outer test subject/record 互斥。V2 核验 outer/inner 的 source-slot、text fold 与 atomic material group 隔离；不得只看计数。
3. V3 调用 H 的禁止字段断言，覆盖 current/future/target tokens、target statistics、candidate payload 与 ET；记录 H_full/H_empty。H feature 只允许登记的 probe/history 接口；candidate 侧只允许 source-sentence identity exclusion。
4. V4 核对 base+derived 六个 artifacts 与两个 audit 的 physical/canonical/provenance bindings；逐项确认 outer/inner scope、source-slot、target、L=5、first-nine prefix、target position、paired 1:1/1:9、eligible/excluded ledger、scoring_only=true、training_records_removed=0。不得改写大 JSON。
5. V5 实现一个与模型无关的 run-ledger schema/validator。每个 stage 至少记录 stage_type、outer_cell、inner_cell（适用时）、fit_record_ids、selection_record_ids、outer_test_record_ids_read、calibration_record_ids、input artifact hashes。规则：fit IDs 是对应 train 子集；inner validation 只能在 selection_record_ids；outer test 不得出现在 fit/selection/threshold/tuning；test calibration count 必须为 0。当前 real audit outcome 只能是 PASS_PRE_RUN_CONTRACT，并写 future_run_admission_required=true、real_training_ledgers_audited=0。
6. JSON 顶层至少有 schema_version、audit_scope、input_bindings、checks.V1...V5、overall_outcome、future_run_admission_required、real_training_ledgers_audited、limitations。markdown 只摘要 JSON 和未来 run admission 要求，不复制大 ledger。

必须测试：
1. 对真实 artifacts 运行 V1–V4 并 PASS；V5 schema/validator 的合法 synthetic ledger PASS，最终 JSON 为 PASS_PRE_RUN_V1_V5，其中 V5 子结果严格为 PASS_PRE_RUN_CONTRACT。禁止把 V5 写成真实训练泄漏已核验。
2. adversarial synthetic tests 至少逐类注入并拒绝：outer subject/record 进入 train；inner record 不属于 outer train；inner validation 进入 fit；outer-test stimulus/material group 进入 train/selection；atomic group 跨边界；H current/future/target-stat/candidate/ET 字段；base 或 derived hash/provenance/scope/target/H/prefix/position/common-support/paired/scoring-only mutation；fit IDs 越界；outer test 用于 selection/threshold/tuning；nonzero test calibration。
3. 运行新 focused leakage tests；既有 test_candidate_common_support.py、test_candidates.py、test_joint_split.py、test_inner_split.py、test_h_definition.py、test_zuco2_source_join.py；完整 unittest suite 一次；scripts/check_project_state.py；scripts/project_status.py；git diff --check。记录精确 pass/skip/fail。若服务器环境依赖失败可如实报告，但真实 V1–V4、V5 adversarial focused tests 与 state checks 不得跳过。

PASS 后唯一允许的状态迁移：
1. S0_LEAKAGE_AUDIT READY→DONE，completion_outcome=PASS_PRE_RUN_V1_V5，写两个 artifacts、三项 code/tests、input hashes、V1–V4 real PASS、V5 PASS_PRE_RUN_CONTRACT 与 future-run revalidation boundary。
2. S0_A1_ADMISSION BLOCKED→READY，blocked_reason 清空；recommended_next_task=S0_A1_ADMISSION；last_completed_task=S0_LEAKAGE_AUDIT。S0_ALIGN_UNIT_COST 仍 BLOCKED，直到 S0_A1_ADMISSION=DONE。
3. 只更新 PROJECT_STATE.yaml、TASKS.yaml、HANDOFF.md、AI_START_HERE.md 并新增一个 run 记录；SPEC 科学规则不改。提交并 push。
4. 最终报告 commit SHA、变更文件、audit hashes、V1–V5 子结论、adversarial tests、完整测试计数、状态迁移和下一任务。不得报告 EEG、训练性能、held-out retrieval、Gate、route、主实验或 ROAMM 结论。

若纯工程命名/排版/helper API 有细小差异，在不改变上述 audit semantics、状态边界或不可变证据的前提下自行做最小决定并继续。若真实协议 artifact、人口、候选、隔离关系或 hash 不一致，必须停止报告，不能自行宽容。
```

本次 v3.12 审查只准入已生成的 text/protocol common-support 证据并冻结 pre-run leakage audit；没有读取或生成 EEG 数值、模型训练、held-out retrieval、Gate、route 或论文级结果。

---

## 附录 Q：v3.13 泄漏准入、A1 可识别性修正与下一项 Codex 指令

### Q.1 对提交 `d9dfe51442155fbd3854d223916c519a7757fff1` 的审查结论

`S0_LEAKAGE_AUDIT=DONE/PASS_PRE_RUN_V1_V5` 正式准入，不重开、不返工：

- V1 在 60 个 outer cells、540 个 inner cells 上通过 subject/record isolation；V2/V3 的 source/material 与 H 边界通过。
- V4 核验 190 scopes、18,475 targets、92,375 repeats、17,061 eligible 与 1,414 exclusions，保持 `training_records_removed=0`。
- V5 只准入可执行合同：`future_run_admission_required=true`、`real_training_ledgers_audited=0`；任何未来真实 run 仍要携带自己的 V5 ledger。
- 独立复跑 focused leakage 19/19，真实 audit 重建与正式 JSON/markdown byte-identical；服务器记录 affected 50/50、full suite 157/157。本地缺失 `torch/h5py` 的 import-only 项不构成代码返工理由。
- 机器 JSON SHA256 为 `28f416a4470d8223294e100e2c8dbb514c05d98184a9dd7936c43267d9e8ca2c`，markdown SHA256 为 `8732d08d6b0145b2da9a71976ec44738ffd241e06dfdc8f6ae838080df44e09d`。

### Q.2 A1 phase randomization 的无 outcome 修正

A1 对每个 epoch 去均值、乘 Hann window、计算 rFFT magnitude squared 并对八个频带积分。必须区分 phase rotation 的位置：在 A1 实际读取的 windowed rFFT 上保持 magnitude 只改 phase，bandpower 恒等；在原 epoch 上先改 phase、inverse-rFFT、再进入 Hann frontend，窗函数会使新的实际分析谱 magnitude 改变，因此它已经不是“只改变 A1 未使用的 phase”。一个版本是 identity，另一个版本混入 magnitude perturbation，旧 `phase-only` 条款没有唯一、可解释且可识别的 A1 null 语义。

D32 的改动完全发生在 EEG outcome、训练与 Gate 前。分析谱 phase rotation 仍保留，但角色是**不变性实现检查**；A1 的强 sham 改为三种保持容量且明确改变 A1 input 的配对/位置/空间扰动。今后如更换为直接保留 phase 的 backbone，必须在结果前另行冻结其 sham，不能把本条机械外推。

### Q.3 唯一推荐的下一项 Codex 任务：`S0_A1_SOURCE_ADMISSION`

把下面整段连同 v3.13 ZIP 交给 Codex。附录 K–P 的旧 next-task 指令均已完成或失效，不得重复执行。

```text
任务：基于 origin/main 的 d9dfe51442155fbd3854d223916c519a7757fff1，先用提供的 v3.13 ZIP 更新六个规格/状态文件，然后只完成 ZuCo 2.0 NR/TSR 的 S0_A1_SOURCE_ADMISSION。目标是用真实 release 数据证明 A1 的 source/field/sampling/channel/unit/finite/identity/coverage 合同并做确定性 feature smoke。不要拟合 normalization，不要训练 probe/sham/alignment model，不要运行 A-A1–A-A4、unit cost、direct u+、Stage 1、Gate、route、main experiment 或 ROAMM。

开始前与 ZIP 导入：
1. git fetch origin；确认工作分支包含且 HEAD 基线为 d9dfe51442155fbd3854d223916c519a7757fff1。若存在未知用户修改，保留并报告，不覆盖。
2. 列出 ZIP entries；拒绝绝对路径、`..`、symlink、重复路径、大小写冲突或清单外文件；核对我随 ZIP 给出的整体 SHA256。只允许导入：
   - guide/EEG_Text_Bprime_Unified_Paper_Spec_v3_13_2026-08-15.md
   - PROJECT_STATE.yaml
   - TASKS.yaml
   - HANDOFF.md
   - AI_START_HERE.md
   - runs/2026-08-15_024_v313_leakage_review_a1_source_freeze.md
3. 按 AI_START_HERE.md 恢复状态，运行 scripts/check_project_state.py 与 scripts/project_status.py。预期：31 tasks / 16 done；S0_LEAKAGE_AUDIT=DONE/PASS_PRE_RUN_V1_V5；S0_A1_SOURCE_ADMISSION 是唯一 READY/recommended；S0_A1_ADMISSION 与 S0_ALIGN_UNIT_COST BLOCKED；ROAMM deferred。任何不一致报 STATE_SPEC_CONFLICT，不猜测修复。
4. 读取 SPEC v3.13 顶部覆盖条款、D32–D34 与附录 Q，打印 PROJECT SNAPSHOT 后才编码。d9dfe51 的 leakage code/artifacts 不修改。

允许新增/修改：
- 新增 02_code/src/data/a1_source_admission.py
- 新增 02_code/scripts/admit_zuco2_a1_source.py
- 新增 02_code/tests/test_a1_source_admission.py
- 新增 artifacts/a1_real_source_contract.yaml
- 新增 01_data_protocol/a1_source_exclusions.jsonl.gz
- 新增 04_results/audits/zuco2_a1_source_admission.json
- 最小修改 02_code/src/backbones/a1_spectral.py、02_code/tests/test_a1_contract.py、artifacts/a1_frontend_freeze.yaml：将现有“finite fraction >=0.95 后 nan/inf 置零”改为任何非有限样本立即拒绝；在 config/freeze 中绑定 `finite_policy=reject_any_nonfinite_no_imputation`，更新真实 implementation/config hashes 和 synthetic self-check。不要改变频带、Hann-periodogram、nFFT、窗口、stride、840D 顺序、normalization 公式、encoder 或 d_align。
- 更新 PROJECT_STATE.yaml、TASKS.yaml、HANDOFF.md、AI_START_HERE.md，并新增一个唯一 runs 记录。不要改 SPEC 科学规则。

真实只读源审计：
1. 从现有 dataset card、extraction manifest、source-slot join 和 loader 恢复服务器实际 release 路径；不得下载、复制或提交 EEG。遍历全部 36 个 ZuCo2 NR/TSR task-summary MAT，记录 path-relative identity、size、physical SHA256、task、subject、release/source provenance。另遍历能证明预处理的 release metadata/scripts 与 `Preprocessed` EEG 文件，但只记录紧凑证据/hash，不提交源数据。
2. `sentenceData.rawData` 只接受 release-native `[samples,105]` 非空 numeric matrix；word `rawEEG` 的每个有效 fixation reference 也只接受 `[samples,105]`。`[105,samples]` 不在 source admission 中自动转置；rank/axis 104、106、object placeholder、空值、dangling reference 与任何 NaN/Inf 都显式 ledger，绝不填补或静默跳过。已有 generic orientation helper 不能放宽 source-field contract。
3. 采样率 500 Hz 必须同时有官方 acquisition 说明和 release/preprocessing metadata 或脚本证据；不得从 array length 猜测。逐文件记录一致性；mixed/unverified 时 outcome=`SOURCE_SAMPLING_UNVERIFIED` 并停止在 source task。
4. 从 release `Preprocessed` 文件的 `EEG.chanlocs.labels` 提取 ordered 105-label tuple，要求标签唯一，并在所审文件中 exact identical；记录 tuple 与 SHA256。还必须从 release 生成脚本/metadata/数据字典证明 task-summary `rawData/rawEEG` 的第 2 轴使用这一相同顺序。只证明“有 105 列”不够；若无法证明，outcome=`SOURCE_ORDER_UNVERIFIED`，不得用 first-105、空间近邻或数值模式猜测。
5. 单位按 D34：优先记录 release metadata/script 的显式 amplitude unit；没有显式 label 时不得由 magnitude 或论文的 90 µV artifact criterion 反推。只有在 release evidence 能证明 summary/word EEG 未另行数值换算、属于同一稳定 preprocessing scale/order 时，才允许 `amplitude_unit_status=release_native_amplitude_unit_unlabelled`，不做 V/µV 换算，PSD 写 native-unit²，并声明后续只可 outer-train robust normalization。若稳定 scale/provenance 也无法证明，outcome=`SOURCE_SCALE_UNVERIFIED`。
6. 每个 sentence/word reference 必须绑定已准入 `(task, subject, source_slot/record_id)`；拒绝 duplicate identity、task/subject/source-slot mismatch。排除 ledger 覆盖每个无效 reference，字段至少含 task、subject、source_slot、source_kind、reference locator、reason。gzip 必须 deterministic（mtime=0）；不得提交大数组、feature cache 或重复 protocol JSON。
7. 预先固定 coverage/G0：每个 task 至少保留 12 subjects；每个 retained task×subject 至少有一个有效 word source，以及一个不少于 500 samples 的 valid sentence rawData 供固定窗。报告所有 task/subject/source-kind 的 valid/excluded counts、shape/duration summaries 和有限值/数值 quantiles（quantiles 只作诊断，禁止据此选阈值）。未达 G0 则 outcome=`SOURCE_COVERAGE_G0_FAIL`。

确定性 feature smoke 与 phase 诊断：
1. 用 seed 20260813 对 `(task,subject,source_kind,source_slot,reference_locator)` 的 canonical tuple 做 SHA256 排序；每个 retained task×subject×source_kind 取前两个 valid records（若只有一个则取一个并如实记录），选择规则在读数值分布前固定。
2. 对 word source 调用 A1 word path，对 sentence rawData 调用 1 s/0.5 s fixed-window path。独立运行两次；每条输出必须为 float32、最后一维 840、全有限，且两次 array bytes/hash exact identical。窗口不足 500 samples 的 sentence 只以 `FIXED_WINDOW_LT_500_SAMPLES` 从 fixed smoke 排除，不改 word/source identity。
3. 实现 deterministic **analysis-spectrum phase rotation**，仅用于测试：对 epoch 先做与 A1 完全相同的去均值、Hann、nFFT 与 rFFT，保持其 magnitudes、合法旋转 phase、保留 DC/Nyquist 实值约束，然后直接从 rotated-spectrum magnitude² 用相同频带积分得到 features。不要 inverse-rFFT 后再次进 Hann frontend。合成 finite epoch 与上一步真实 smoke 都须在 `rtol=1e-5, atol=1e-7` 内与原 features 相等；报告最大绝对/相对误差。相等是 PASS 预期，不生成 `u_oof_phase_only`，不运行 real-vs-sham likelihood，也不要把 pre-window phase perturbation 混入本任务。
4. source task 不得 fit RobustFeatureNormalizer。若代码路径意外调用 `.fit()`、torch model、text encoder、candidate score 或 outer-test metric，测试必须失败。

输出合同：
1. `artifacts/a1_real_source_contract.yaml` 至少绑定 spec/commit/run、36-file inventory hash、source fields/shapes、500 Hz evidence、ordered 105-label tuple/hash、unit status/evidence、strict finite policy、identity join hashes、coverage/G0、smoke selection/version/hashes、phase tolerance与 outcome。
2. `04_results/audits/zuco2_a1_source_admission.json` 只存紧凑 aggregates/bindings/checks/limitations，不内嵌 EEG 或逐样本数值；顶层至少有 schema_version、baseline_commit、input_bindings、checks、coverage、exclusion_reason_counts、feature_smoke、phase_invariance、overall_outcome、limitations。
3. `01_data_protocol/a1_source_exclusions.jsonl.gz` 只存逐 reference 排除，不存数组。排序与 gzip bytes 两次构建必须完全一致。
4. JSON/YAML 均记录 `no_normalizer_fit=true`、`no_model_or_probe_training=true`、`no_heldout_metric=true`、`no_eeg_array_committed=true`。不要把 source PASS 写成 A1 signal admission、Gate A 或论文结果。

必须测试：
1. focused synthetic mutations 至少拒绝：缺/错 field、placeholder/dangling ref、rank 错、104/106 channel、source transpose、NaN/Inf、mixed/unverified srate、重复/变化/乱序 channel labels、summary order provenance 缺失、非法 unit inference/conversion、unstable native scale、duplicate identity、task/subject/source-slot mismatch、非确定性 ledger/smoke、normalizer/model 调用。
2. 增加 A1 frontend 回归：一个 NaN、+Inf、-Inf 均立即失败且没有 nan_to_num；既有 finite feature/word/fixed/encoder tests 保持通过；合成 phase-invariance 通过。
3. 在真实 server data 上运行 admission 两次，三项正式输出 canonical/byte identical；复核输入 physical hashes 未变。运行新 focused source tests、test_a1_contract.py、test_zuco2_loader.py、test_zuco2_source_join.py、test_joint_split.py、test_leakage_audit.py；完整 unittest suite 一次；scripts/check_project_state.py、scripts/project_status.py、git diff --check。记录精确 pass/skip/fail。环境依赖可如实报告，但 real 36-file audit、channel/order/evidence、strict finite、identity、G0、feature determinism 与 focused mutations 不能跳过。

唯一允许的状态迁移：
1. 只有全部硬验收 PASS，S0_A1_SOURCE_ADMISSION READY→DONE，completion_outcome=`PASS_REAL_A1_SOURCE`，写 completed_by_run/evidence/hashes/counts/limitations；S0_A1_ADMISSION BLOCKED→READY 并成为唯一 recommended；last_completed_task=S0_A1_SOURCE_ADMISSION。S0_ALIGN_UNIT_COST 仍 BLOCKED。
2. 若 sampling/order/scale/G0/identity/finite 任一失败，不把 source task 标 DONE；记录明确 outcome/blocker 与完整 audit，recommended_next_task 置为 null 或明确 AUTHOR_REVIEW_A1_SOURCE，停止。不得猜 channel/unit、放宽 finite/G0、删 subject 或继续 A-A1–A-A4。
3. 提交并 push。最终报告 commit SHA、变更文件、三个正式 source artifact SHA256、36-file inventory/counts、channel tuple hash、sampling/unit status、coverage/exclusions、smoke/phase errors、测试计数、状态迁移和下一任务。不得报告 EEG 性能、训练、held-out retrieval、Gate、route、main experiment 或 ROAMM 结论。

工程命名、compact summary 排版或 helper API 若有不改变上述 source identity/order/unit/finite/G0/determinism 的细小差异，可自行做最小决定继续；任何科学边界差异必须停止，不能为了 PASS 宽容。
```

本次 v3.13 审查准入的是 leakage artifact/contract，并在任何 EEG outcome 前修正 A1 sham 的可识别性与冻结真实源门；没有产生 EEG 性能、训练、held-out retrieval、Gate、route 或论文级结论。
